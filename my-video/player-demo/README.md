...
- **Interface props**, on a second `<Player>` of the reel: `autoPlay` with `initiallyMuted`, `loop`, `showVolumeControls`, `allowFullscreen`, `clickToPlay={false}`, `doubleClickToFullscreen`, `spaceKeyToPlayOrPause={false}`, `initiallyShowControls`, `alwaysShowControls`, `hideControlsWhenPointerDoesntMove`, `numberOfSharedAudioTags`, `sampleRate`, `bufferStateDelayInMilliseconds`, `showPosterWhenBuffering`/`showPosterWhenBufferingAndPaused`, `renderLoading`, `errorFallback`, all five `render*` control props (play/pause, mute, fullscreen, volume slider, custom controls), `browserMediaControlsBehavior`, `volumePersistenceKey`, `overrideInternalClassName`, `overflowVisible`, `noSuspense` and `logLevel`. The thumbnails pass `renderLoading`, `errorFallback`, `overflowVisible`, `noSuspense` and `logLevel` too.
- **`errorFallback` in action**, on a small third `<Player>` whose component throws from frame 5 and which opens on frame 5. The error still shows in the console, because React logs errors that a boundary catches.
...
- **Console:** apart from the third Player's deliberate error, the only message is the Player's license notice, a `console.warn` ("Some companies are required to obtain a license…"). Passing the `acknowledgeRemotionLicense` prop to `<Player>` silences it. That's for the project owner to decide, so the demo doesn't pass it.
