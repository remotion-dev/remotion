...
- **Console:** apart from the third Player's deliberate error, the only other message would be the Player's license notice, a `console.warn` ("Some companies are required to obtain a license…"). The owner uses Remotion's free license, so all three Players pass `acknowledgeRemotionLicense`, which hides that notice.
