...
- **`<TransitionSeries.Overlay>` doesn't shorten the timeline.** Unlike a `<TransitionSeries.Transition>`, an overlay sits centered on the cut between two sequences and takes no frames away. `FullReel` has two, so its duration formula subtracts 15 frames for every cut *except* the overlays: `SCENE_COUNT * 75 + EFFECTS_CATALOG_DURATION - (SCENE_COUNT - OVERLAY_COUNT) * 15` = 24 × 75 + 390 − 22 × 15 = 1860 (25 sequences, so 24 cuts).
...
- **`useRemotionEnvironment()` / `getRemotionEnvironment()` return `isStudio: undefined` during a CLI render**, not `false`, even though the type says `boolean`. `isStudio`, `isPlayer` and `isReadOnlyStudio` are copied straight from `window.remotion_*` globals that are never set while rendering (`packages/core/src/get-remotion-environment.ts`). Check them for truthiness, never `=== false`. `CoreEnvironmentScene` prints the raw value, which is why its readout says `isStudio=undefined`. This fork's `packages/core` now coerces all three flags with `Boolean()`, but my-video uses the published 4.0.527, which doesn't.
- **Every rough-notation annotation except `<Box>` (7) defaults to `strokeWidth` 20.** At that width `OutroScene`'s strike-through and cross-out hid "manual editing" and "timelines" completely, and the underline and bracket were solid bars, so every annotation there now sets its own. `<Circle box>` defaults to `"around"` (an ellipse circumscribing the text); `OutroScene` uses `"inside"`, which fits the ellipse inside the padded box, so its padding is what keeps the line clear of the words.
- **rough-notation `seed`** changes the generated shape; a new value every few frames (`seed={Math.floor(frame / 4)}` on `OutroScene`'s `<Box>`) gives the "boiling" hand-drawn look. **`disabled`** removes only the mark: `TitleScene` wraps every subtitle word in `<Highlight>` and disables all but two, so each word keeps the same structure.
- **rough-notation's `<Sequence>` props** (`name`, `showInTimeline`, `from`, `durationInFrames`, `trimBefore`, `freeze`, `hidden`) go to a `<Sequence layout="none">` that wraps the annotated text too (`packages/rough-notation/src/Annotation.tsx`). `TitleScene` and `OutroScene` set `name` and `showInTimeline`, which only change the Studio timeline. The rest aren't used: `from`, `durationInFrames` and `hidden` would unmount the words themselves, not just the mark, and `freeze`/`trimBefore` change nothing visible because `progress` is a prop computed outside.
...
`EffectsCatalogScene` shows all 74 effects in `@remotion/effects`' catalog (`EFFECT_CATALOG` in `@remotion/studio-shared`, which generates remotion.dev/docs/effects/api), six per page. Found while building it:

- **remotion.dev/docs/api doesn't list `@remotion/effects`.** The effects have their own index at `/docs/effects/api`, so a coverage check that only reads `/docs/api` misses all of them. Until this scene, the showcase used 8 of 74.
- **Each component with effects costs two WebGL2 contexts.** Core's `CanvasPool` gives every effect chain its own pair of scratch canvases (per backend), kept for the component's lifetime while its size doesn't change; switching `effects` reuses them. Chrome keeps at most 16 WebGL contexts alive per page and then loses the oldest, which cancels the render ("WebGL context was lost during canvas effect rendering"). Measured here with N `<CanvasImage>`s each running an animated `blur()`: a 30-frame render passes at 8 (16 contexts) and fails at 9 (18). A single still with 20 passes, because the error is only raised when a chain runs again on a lost context, so test a context budget with a multi-frame render, not `remotion still`. So the catalog keeps six tiles mounted, keyed by slot, and swaps their `effects` and `src` each page, and `FullReel` joins it to `EffectsScene` with a `<TransitionSeries.Overlay>` rather than a transition, so the two scenes are never mounted together.
- **Parameters are validated when the descriptor is created**, not during the render, so calling every entry's factory under Bun (`EFFECTS_CATALOG.forEach((e) => e.effects(t))` for each `t`) catches range errors without rendering. That found the one bad value: `brightness({amount})` is additive, from -1 to 1 (default 0), unlike `contrast()`/`saturation()`, which multiply.
- **Choose the source to suit the effect.** `roughenEdges()`, `outline()` and `dropShadow()` work on the alpha boundary, so they need a subject on transparency; on the opaque `sample-frame.png` they show nothing. `glow()` and `lightTrail()` threshold brightness instead, so they work on the photo too. `regionBlur()`'s smear is hard to see on the photo's smooth gradient, so the catalog uses the star for it.
- **Check each parameter's unit in the docs.** `zoomBlur({amount})` is in pixels (default 40), not 0–1; the catalog first animated it 0 → 0.6, and its tile showed no blur at all. The first version of this note blamed the source image instead.
...
- **`separateVideoLayers()` checks that the video decodes before it loads the model**, so given the H.264 `.mp4` it fails here even with a model; `VideoMattingScene` uses the `.webm`.
...
- **Node side, from `scripts/renderer-apis.mjs`:**
  ...
  - `renderMedia()`'s defaults in 4.0.527 write `yuvj420p` tagged `bt470bg` with a single keyframe; `pixelFormat`, `colorSpace` and `gopSize` change that, and the script asserts each.
  - An h264 render with `enforceAudioTrack` reads about 0.07 s longer than its frames (1.069 s for 30 frames at 30 fps) because of AAC priming, so a duration check needs more tolerance there.
  - `renderStill({frame: 1020, onArtifact})` on ExtendedReel reports the artifact's `frame` as 0.
