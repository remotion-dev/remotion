...
- **Choose the source to suit the effect.** `roughenEdges()`, `outline()`, `dropShadow()` and `glow()` work on the alpha boundary, so they need a subject on transparency; on the opaque `sample-frame.png` they show nothing. `zoomBlur()` and `regionBlur()` also looked like no-ops on it: it's a conic gradient, constant along every ray from the center, so there is nothing to smear.
