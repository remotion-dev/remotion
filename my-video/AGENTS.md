...
- **Each component with effects costs two WebGL2 contexts.** Core's `CanvasPool` gives every effect chain its own pair of scratch canvases (per backend), kept for the component's lifetime while its size doesn't change; switching `effects` reuses them. Chrome only keeps a limited number of WebGL contexts alive per page and then loses the oldest, which cancels the render ("WebGL context was lost during canvas effect rendering"). So the catalog keeps six tiles mounted, keyed by slot, and swaps their `effects` and `src` each page, and `FullReel` joins it to `EffectsScene` with a `<TransitionSeries.Overlay>` rather than a transition, so the two scenes are never mounted together.
