...
Build the uploadable skill bundle from this folder:

```console
# macOS/Linux
mkdir -p /tmp/remotion-video && cp chat-skill/SKILL.md /tmp/remotion-video/ && cp -R .claude/skills/. /tmp/remotion-video/ && (cd /tmp && zip -rq remotion-video-skill.zip remotion-video) && mv /tmp/remotion-video-skill.zip .
