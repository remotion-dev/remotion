...
- `src/brand/` — the FinHub brand kit, to reuse in every video: `BilingualCaption`, `LowerThird`, `BadgeRow`, `LenderRow`, `NotoEmoji`, `EndCard`, Finance Hub's colours and Be Vietnam Pro in `theme.ts`, and `BrandOverlay`, a transparent logo + lower-third overlay that renders ProRes 4444 for video editors
