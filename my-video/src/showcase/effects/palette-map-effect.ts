...
// A second custom createEffect() filter, next to sepia-effect.ts in
// CoreMediaScene: every pixel moves toward the nearest colour of `palette`,
// by `amount`. Its `palette` param is an `array` schema field of `color`
// items, which the Studio shows as an editable list (new entries start at
// newItemDefault, and at least minLength stay).
