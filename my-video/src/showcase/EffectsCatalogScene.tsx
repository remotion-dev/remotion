...
export const EFFECTS_CATALOG: readonly CatalogEntry[] = [
  ...
  {name: "contrast()", category: "Color", source: "photo", effects: (t) => [contrast({amount: at(t, 0.5, 1.8)})]},
  {
    ...
    category: "Color",
    source: "photo",
    effects: (t) => [colorCorrection({exposure: 0.25, contrast: 1.1, shadows: 0.2, highlights: -0.15, temperature: at(t, -0.6, 0.6), vibrance: 0.2})],
  },
  {name: "colorKey()", category: "Color", source: "greenscreen", effects: (t) => [colorKey({keyColor: "#00ff00", similarity: at(t, 0, 0.3)})]},
  ...
  {name: "outline()", category: "Blur & Shadow", source: "subject", effects: (t) => [outline({width: at(t, 1, 6), color: "#facc15"})]},
  {
    ...
    category: "Blur & Shadow",
    source: "subject",
    effects: (t) => [lightTrail({direction: 180, distance: at(t, 0, 60), intensity: 1.6, decay: 0.9, threshold: 0.2, color: "#ffb000"})],
  },
  ...
  {name: "noise()", category: "Stylize", source: "photo", effects: (t) => [noise({amount: at(t, 0.1, 0.5), seed: 7})]},
  {
    ...
    category: "Stylize",
    source: "photo",
    effects: (t) => [noiseDisplacement({center: [0.5, 0.5], radius: 0.3, strength: at(t, 0, 24), seed: 12, grainSize: 4, passes: 6, feather: 0.25})],
  },
  {
    name: "paper()",
    category: "Stylize",
    source: "photo",
    effects: (t) => [
      paper({amount: at(t, 0.3, 1), roughness: 0.4, fiber: 0.3, fiberSize: 0.2, crumples: 0.3, crumpleSize: 0.35, folds: 0.65, foldCount: 5, drops: 0.2, scale: 0.6, seed: 6}),
    ],
  },
  {name: "roughenEdges()", category: "Stylize", source: "subject", effects: (t) => [roughenEdges({amount: at(t, 0.2, 1), border: 12, scale: 0.07, seed: 231.2})]},
  {
    ...
    category: "Stylize",
    source: "photo",
    effects: (t) => [pattern({scale: 0.3, gapX: 8, gapY: 8, offsetU: t * 0.5, rowOffset: 30, origin: [0.5, 0]})],
  },
  ...
  {name: "scanlines()", category: "Stylize", source: "photo", effects: (t) => [scanlines({amount: 0.4, spacing: 4, thickness: 1, offset: t * 20})]},
  ...
  {name: "liquidContours()", category: "Generate", source: "photo", effects: (t) => [liquidContours({spacing: 30, scale: 150, seed: 4, offsetX: 13.4, phase: 3.23 + t})]},
  ...
  },
  {
    ...
    source: "photo",
    effects: (t) => [
      halftoneLinearGradient({firstStopDotSize: 0, secondStopDotSize: at(t, 4, 22), firstStopPosition: [0, 0.5], secondStopPosition: [1, 0.5], gridSize: 12, colorMode: "source"}),
    ],
  },
  {
    name: "gridlines()",
