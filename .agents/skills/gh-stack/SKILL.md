...
| `gh stack merge <target> --yes --squash`  | `gh pr merge`       | `gh pr merge` cannot merge a stack            |
...
Only run a stack merge when the user explicitly asks to merge. Inspect `gh stack view --json`
first, resolve whether the user wants the whole stack or only through a specific PR, and scope the
merge with an argument plus an explicit method:

```bash
gh stack merge 42 --yes --squash # PR #42 plus every unmerged PR below it
...
Although omitting a method reuses the last-used method, agents must pass `--squash`, `--merge`,
`--rebase`, or `--merge-method <method>` explicitly. If the base branch uses a merge queue, the
stack is queued instead and the queue picks the method, ignoring the flag with a warning; queued
PRs may land in separate groups.
