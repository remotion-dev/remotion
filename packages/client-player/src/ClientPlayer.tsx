...
const PlayerFn = <T,>(
	...
	if (componentProps.defaultProps !== undefined) {
		throw new Error(
			'The <ClientPlayer /> component does not accept `defaultProps`, but some were passed. Use `inputProps` instead.'
		);
	}
	...
	Internals.validateDurationInFrames({
		durationInFrames,
		component: 'of the <ClientPlayer/> component',
		allowFloats: false,
	});
	Internals.validateFps(
		fps,
		'as a prop of the <ClientPlayer/> component',
		false
	);
	...
	if (typeof window !== 'undefined') {
		// eslint-disable-next-line react-hooks/rules-of-hooks
		useLayoutEffect(() => {
			// Inject CSS only on client, and also only after the ClientPlayer has hydrated
 ...
 */
export const ClientPlayer = forward(PlayerFn);
