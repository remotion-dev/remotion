...
export const isMultipleFrameRanges = (
	frameRange: FrameRange,
): frameRange is FrameRangeTuple[] => {
	return Array.isArray(frameRange) && Array.isArray(frameRange[0]);
};

const validateFrameRangeTuple = (frameRange: FrameRangeTuple) => {
	...
	if (isMultipleFrameRanges(frameRange)) {
		...
		for (let index = 0; index < frameRange.length; index++) {
			...
			if (previous && previous[1] !== null && range[0] <= previous[1]) {
				throw new Error(
					`Frame ranges must be ordered and non-overlapping, but got ${previous.join('-')} followed by ${range.join('-')}.`,
				);
			}
		}
