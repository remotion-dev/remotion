...
pub fn extract_audio(input_pat: &str, output_path_: &str) -> Result<(), ErrorWithBacktrace> {
    ...
    let input_path = "/Users/jonathanburger/audio-concatenation-research/tones/tone.m4a";
    let output_path = "/Users/jonathanburger/audio-concatenation-research/tones/rust.aac";
        ...
        loop {
            match ictx.get_next_packet() {
                Ok((stream, mut packet)) => {
                    ...
                    packet.set_dts(Some(packet.dts().unwrap()));
                    packet.set_pts(Some(packet.pts().unwrap() + 1024));
                    ...
                    _print_verbose(&format!("packet {:?} {:?}", packet.dts(), packet.pts()));
