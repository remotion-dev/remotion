  ...
  for (const videoCodec of videoCodecs) {
    for (const audioCodec of audioCodecs) {
      ...
      if (!(await Bun.file(path.join(outDir, outputName)).exists())) {
        ...
        // 10 second video
        await $`ffmpeg ${videoEncoder ? "-i" : ""} ${
          videoEncoder ? baseMute : ""
        }  ${audioEncoder ? "-i" : ""} ${
          audioEncoder ? "tone.wav" : ""
        } -t 10 -c:v ${videoEncoder} -c:a ${audioEncoder} ${path.join(
          outDir,
          outputName
        )} -y`;
      }
...
variants.push({
  ...
  size: Bun.file(path.join(outDir, "dialogue.wav")).size,
  category: "audio",
});
...
variants.push({
  ...
  size: Bun.file(path.join(outDir, "greenscreen.mp4")).size,
...
variants.push({
  ...
  size: Bun.file(path.join(outDir, "first-frame-at-4sec.webm")).size,
  category: "edge-cases",
});
