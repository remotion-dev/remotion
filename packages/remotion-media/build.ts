#!/usr/bin/env bun
import { build, Glob, S3Client, type BuildConfig } from "bun";
...
if (!Bun.env.AWS_ACCESS_KEY_ID || !Bun.env.AWS_ACCESS_SECRET_KEY) {
  console.log(
    "Skipping R2 upload: AWS_ACCESS_KEY_ID and AWS_ACCESS_SECRET_KEY are not set",
  );
} else {
  const client = new S3Client({
    accessKeyId: Bun.env.AWS_ACCESS_KEY_ID,
    secretAccessKey: Bun.env.AWS_ACCESS_SECRET_KEY,
    endpoint:
