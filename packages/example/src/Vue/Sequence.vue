...
const durationInFrames =
	(props.durationInFrames as number | undefined) ?? Infinity;
const layout = (props.layout as 'absolute-fill' | 'none') ?? 'absolute-fill';
const showLoopTimesInTimeline = props.showLoopTimesInTimeline as
	| number
	| undefined;
...
const {value: parentSequence} = inject(
...
const compositionManager = inject(
	'remotion.compositionManager'
) as Ref<CompositionManagerContext>;
...
<template>
	<div class="container">
		<div
			v-if="shouldRender && layout === 'absolute-fill'"
			class="remotion__absolute"
		>
			<slot></slot>
		</div>
