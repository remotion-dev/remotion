...
export const lightLeakEffectSchema = lightLeakSchema;
...
export type LightLeakEffectParams = LightLeakParams;
...
const resolve = (p: LightLeakParams): LightLeakResolved => ({
	seed: p.seed ?? DEFAULT_SEED,
	hueShift: p.hueShift ?? DEFAULT_HUE_SHIFT,
	progress: p.progress ?? DEFAULT_PROGRESS,
});

const validateLightLeakParams = (params: LightLeakParams): void => {
...
export const lightLeak = createEffect<LightLeakParams, LightLeakGlState>({
