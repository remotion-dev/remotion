...
void main() {
	...
	float radius = length(delta) / max(length(uResolution), 0.001);
	float rayPosition = angle * uRayCount;
	float ray = pow(max(0.0, 0.5 + 0.5 * cos(rayPosition)), uSharpness);
	float rayIndex = floor(rayPosition / 6.28318530718);
	float variation = mix(0.55, 1.25, hash(rayIndex));
	float streak = ray * variation;
...
export const laserRays = createEffect<LaserRaysParams, LaserRaysState>({
	...
	calculateKey: (params) => {
		const r = resolve(params);
		return `laser-rays-${r.color}-${r.backgroundColor}-${r.center.join('-')}-${r.rayCount}-${r.sharpness}-${r.intensity}-${r.amount}-${r.rotation}-${r.radiusFalloff}`;
	},
