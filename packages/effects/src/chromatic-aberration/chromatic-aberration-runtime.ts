...
} from './chromatic-aberration-shaders.js';
...
export const cleanupChromaticAberration = (
...
const drawFullscreenQuad = (state: ChromaticAberrationState): void => {
	const {gl, vao} = state;
	gl.bindVertexArray(vao);
...
export const applyChromaticAberration = ({
	...
	if (uniforms.uOffset) gl.uniform2f(uniforms.uOffset, x, y);
