...
import {ZOOM_BLUR_FS, ZOOM_BLUR_VS} from './zoom-blur-shaders.js';
...
export const cleanupZoomBlur = (state: ZoomBlurState): void => {
...
const drawFullscreenQuad = (state: ZoomBlurState): void => {
	const {gl, vao} = state;
	gl.bindVertexArray(vao);
