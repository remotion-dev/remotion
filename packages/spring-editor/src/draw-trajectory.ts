...
export const drawTrajectory = ({
	...
	const executeDraw = async () => {
		for (let i = 0; i < springTrajectory.length; i++) {
			...
			if (animate) {
				(document.getElementById('scale') as HTMLElement).style.transform =
					`scale(${springTrajectory[i]})`;
				(document.getElementById('translate') as HTMLElement).style.transform =
					`translateY(${interpolate(springTrajectory[i], [0, 1], [100, 0])}px)`;
				(document.getElementById('rotate') as HTMLElement).style.transform =
					`rotate(${interpolate(
						springTrajectory[i],
						[0, 1],
						[Math.PI * 2, 0],
