...
export const createTimeLineFromStoryWithDetails = (
  ...
  for (let i = 0; i < storyWithDetails.content.length; i++) {
    ...
    // Build text elements from word-level timestamps
    ...
    for (const wordTs of wordTimestamps) {
      ...
      if (candidateText.length > MaxSentenseSizeChars && currentWords.length > 0) {
    ...
    if (currentWords.length > 0) {
      const lastEndMs = wordTimestamps[wordTimestamps.length - 1].end * 1000 + durationMs;
      const textElem: TextElement = {
        startMs: currentStartMs,
