...
async function generateStory(options: GenerateOptions) {
  try {
    ...
    let voiceId = options.voiceId || process.env.TYPECAST_VOICE_ID;

    if (!voiceId) {
      const response = await prompts({
        type: "text",
        name: "voiceId",
        message: "Enter your Typecast voice ID:",
        validate: (value) =>
          value.length > 0 || "Voice ID is required",
      });

      if (!response.voiceId) {
        console.log(chalk.red("Voice ID is required. Exiting..."));
        process.exit(1);
      }
    ...
    for (let i = 0; i < storyWithDetails.content.length; i++) {
      ...
      const wordTimestamps = await generateVoice(
        storyItem.text,
        typecastApiKey!,
        voiceId!,
