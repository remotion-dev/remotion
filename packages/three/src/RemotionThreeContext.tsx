...
export const RemotionThreeContext = React.createContext<RemotionThreeContextType | undefined>(undefined);

export function useRemotionThreeContext() {
    const ctx = React.useContext(RemotionThreeContext);
    if (!ctx) {
        throw new Error('useRemotionThreeContext used outside RemotionThreeContext');
    }
    return ctx;
}
