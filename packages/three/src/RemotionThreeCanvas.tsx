/// <reference path="./effects/postprocessing.d.ts" />
...
import { RemotionThreeContext, RemotionThreeContextType } from './RemotionThreeContext';
import { Internals } from 'remotion';

export default function RemotionThreeCanvas({
    children,
    tabIndex,
    resize,
    id,
    ...
    const [threeDomNodesWrapper, remotionThreeContext] = useCreateRemotionThreeContext();

    const width = wrapperRef.current?.clientWidth;
    const height = wrapperRef.current?.clientHeight;
    const size = React.useMemo(() => !width || !height ? undefined : ({ width, height }), [width, height]);
...
function useCreateRemotionThreeContext() {
    ...
    const remotionThreeContext = React.useState<RemotionThreeContextType>(() => ({
        useDomNode: (node) => {
            ...
            const containerNode = React.useState(() => document.createElement('div'))[0];
            React.useEffect(() => {
                portalRef.current!.appendChild(containerNode);
                return () => {
