...
```tsx
{
  text.split('').map((char, i) => <span style={{opacity: i < typedCount ? 1 : 0}}>{char}</span>);
}
<span>|</span>;
```
...
const caretOpacity = interpolate(frame % CURSOR_BLINK_FRAMES, [0, CURSOR_BLINK_FRAMES / 2, CURSOR_BLINK_FRAMES], [1, 0, 1], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'});
...
<div style={{position: 'relative'}}>
  ...
  <div style={{position: 'absolute', left: 0, top: 0}}>{WORDS[currentIndex]}</div>
</div>;
```
...
```tsx
const typedOpacity = interpolate(frame, [highlightStart - 8, highlightStart + 8], [1, 0], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'});
const finalOpacity = interpolate(frame, [highlightStart, highlightStart + 8], [0, 1], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'});
...
- [Typewriter](assets/text-animations-typewriter.tsx) - Typewriter effect with blinking cursor and dramatic pause
- [Word Highlight](assets/text-animations-word-highlight.tsx) - Spring-animated highlight wipe on a word in a sentence
- [Word Carousel](assets/text-animations-word-carousel.tsx) - Rotating words with crossfade and blur
- [Text Rotation](assets/text-animations-text-rotation.tsx) - Rotating words with dissolve and blur effects
