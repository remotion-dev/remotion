...
`<TransitionSeries.Overlay>` renders children on top of the junction between two sequences **without** shortening the timeline. The overlay is centered on the cut point by default.
...
Overlays do **not** subtract from the total duration. An overlay cannot be adjacent to a transition or another overlay.
