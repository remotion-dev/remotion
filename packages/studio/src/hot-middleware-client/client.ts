 ...
 */
import type { HotMiddlewareMessage } from "@remotion/studio-server/client";
import { StudioServerClientAPIs } from "@remotion/studio-server/client";
import { processUpdate } from "./process-update";

const { hotMiddlewareOptions } = StudioServerClientAPIs;
...
function eventSourceWrapper() {
  ...
  const timer = setInterval(() => {
    if (
      ...
      StudioServerClientAPIs.hotMiddlewareOptions.timeout
    ) {
      handleDisconnect();
    }
  }, StudioServerClientAPIs.hotMiddlewareOptions.timeout / 2);

  function init() {
    source = new window.EventSource(
      StudioServerClientAPIs.hotMiddlewareOptions.path,
    );
...
function createReporter() {
  ...
  function log(type: "errors" | "warnings", obj: HotMiddlewareMessage) {
    ...
    const newProblems = obj[type]
      .map((msg) => {
        return StudioServerClientAPIs.stripAnsi(msg as string);
      })
      .join("\n");
