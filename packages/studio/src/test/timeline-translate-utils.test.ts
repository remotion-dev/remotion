...
test('parseTranslate normalizes floating point noise', () => {
	...
	expect(
		parseTranslate(
			'10.020000000000001px 30.240000000000002px 40.480000000000004px',
		),
	).toEqual([10.02, 30.24, 40.48]);
});

test('serializeTranslate normalizes floating point noise', () => {
	expect(serializeTranslate([0.1 + 0.2, 10.020000000000001, null])).toBe(
		'0.3px 10px',
	);
	expect(
		serializeTranslate([0.1 + 0.2, 10.020000000000001, 20.030000000000001]),
	).toBe('0.3px 10px 20px');
});

test('serializeTranslate limits coordinates to one decimal place', () => {
	expect(serializeTranslate([254.854512, 393.565342, null])).toBe(
		'254.9px 393.6px',
	);
});
