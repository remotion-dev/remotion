...
test('preserves collapsed Effects rows when selecting an effect prop', () => {
	const nodePathInfo = makeNodePathInfo(
		['body', 0],
		['effects', '0', 'intensity'],
	);
...
test('preserves collapsed Effects rows when selecting an effect row', () => {
	const nodePathInfo = makeNodePathInfo(['body', 0], ['effects', '0']);
