...
export const getDefaultOutputBaseName = (
	...
	let decoded = lastPathSegment ?? displayName;
	try {
...
export const validatePublicOutputName = ({
	extension,
	outName,
}: {
	extension: string;
	...
	if (outName.trim() === '') return 'Enter an output file';
	...
	if (outName.includes('\\')) return 'Use forward slashes in the output file';
