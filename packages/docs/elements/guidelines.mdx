...
- Pass `style` and the ref to the rendered element. With `layout="none"`, the Studio discovers the outline automatically; use [`outlineRef`](/docs/sequence#outlineref) to select a specific element. Set the dimensions in the component because Element metadata does not pass them as props.
