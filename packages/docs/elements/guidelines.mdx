...
- Declare packages that Studio may need to install, but do not declare `react`, `react-dom`, or `remotion`; every Remotion project already provides them.
- Declare every external dependency in the central Element definition. Use an exact version only when a non-Remotion package requires one. Remotion packages use the project's installed Remotion version.

## Design it for composition

An Element should be safe to combine with other content using normal Studio layering.

Size it to the smallest useful bounding box. Let the surrounding [`<Sequence>`](/docs/sequence) control its placement and how long it appears in the video. Set both Element dimensions to `null` when it adapts to the composition.
