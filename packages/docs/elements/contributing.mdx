...
Before opening a pull request, render and visually review the Element and its previews:
...
Copy the rendered preview assets to `packages/docs/static/elements` and commit them. Set `image`, `posterUrl`, and `videoUrl` to `/elements/<category>-<slug>-preview.png` and `/elements/<category>-<slug>-preview.mp4`, matching those files. Preview assets let reviewers see the Element in the pull request deployment. After approval, a repository maintainer uploads them to static storage. Keep “Allow edits from maintainers” enabled on the pull request.

{/* Repository maintainers may instead upload previews to remotion.media and update the preview URLs. */}
