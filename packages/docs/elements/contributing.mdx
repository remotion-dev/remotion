...
2. Add its metadata to `packages/docs/src/components/Elements/element-definitions.ts` to register its Studio composition.
...
The following [Agent Skills](https://agentskills.io/) support the contribution workflow:

- The **scaffold-element skill** copies the template and registers the development composition.
- The **submit-element skill** adds the Element to the gallery and prepares it for review.

## Open the pull request

Before opening a pull request, visually review the Element in Studio.

Include:

- The implementation, page, and gallery registration
- What makes it useful
- A working PNG and MP4 preview, committed directly to the pull request
- Links to inspiration, if any

Preview assets let reviewers see the Element in the pull request deployment. After approval, a repository maintainer uploads them to static storage. Keep “Allow edits from maintainers” enabled on the pull request.
...
When ready, [open a pull request](#open-the-pull-request).
