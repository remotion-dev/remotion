...
In `element-definitions.ts`, declare `assets` using the [`createElementPayload()` asset format](/docs/studio-protocol/create-element-payload#assets). Use public-relative paths scoped to your Element. In `installationProps`, reference those paths with [`staticFileRef()`](/docs/studio-protocol/static-file-ref), which documents the API and a complete example.

`installationProps` shallowly overrides `initialProps` only for installation; an overridden object or array replaces that entire prop. Previews use `initialProps` unchanged. For Elements without installed assets, use `assets: []` and `installationProps: null`.

Studio saves the assets into the public directory and generates [`staticFile()`](/docs/staticfile) expressions on the installed caller, leaving the Element source unchanged. This works in both installation modes. To keep an asset remote after installation, leave its URL in `initialProps` without an installation override.
