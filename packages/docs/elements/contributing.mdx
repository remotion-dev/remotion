...
- Render frame-dependent animation in a child below the Sequence. This makes [`useCurrentFrame()`](/docs/use-current-frame) respect `from`, `trimBefore`, `playbackRate`, and `freeze`. If the component that returns the Sequence calls it, it reads the parent clock.
...
Both installation modes support `initialProps`. Put replaceable starter content in `initial-props.ts`, register it with the component in `element-definitions.ts`, and type it with `satisfies ComponentProps<typeof YourComponent>`.

The gallery passes these props to previews, and Studio writes them onto the installed component. [`Interactive.withSchema()`](/docs/interactive-with-schema) makes props editable in the Inspector. Content declared inside the component is not a prop, so users cannot edit it per instance. Keep rendering and animation in the copied Element file.

For `component-owned-sequence`, Studio supplies `from`, `durationInFrames`, and `name`. Pass `width` and `height` in `initialProps` when the component needs them. Element dimension metadata is not passed as component props.

### Include assets

Declare images, audio, and video that should be installed into the user's public directory. These are separate from the PNG and MP4 gallery previews in [Prepare previews](#2-prepare-previews).

Accept each asset URL as a component prop, such as `audioSrc`. In `initialProps`, pass a URL that works in your preview: a locally served asset or, if you prefer, a hosted URL. The preview location does not need to match the installed asset path.

For example, the oscilloscope uses a hosted audio file:

```ts title="elements/audio/oscilloscope/initial-props.ts"
...
In `element-definitions.ts`, register `assets` and override the asset prop for installation with [`staticFileRef()`](/docs/studio-protocol/static-file-ref):

```ts title="Fields in the central Element definition"
...
const elementImplementations = [
  {
    ...
    // Keep the remaining Element metadata here.
  },
];
```

`installationProps` shallowly overrides `initialProps` only when building the installation payload. Unchanged props keep their initial values; an overridden object or array replaces that entire prop. Previews never receive the reference objects. For Elements without installed assets, use `assets: []` and `installationProps: null`.

Studio downloads the URL to `public/elements/audio-oscilloscope/remotion-made-this-picture-move.mp3` and generates this asset prop on the installed caller:

```tsx title="Installed asset prop"
<AudioOscilloscope
  audioSrc={staticFile('elements/audio-oscilloscope/remotion-made-this-picture-move.mp3')}
/>
```

The Element source is copied unchanged. Do not hardcode `staticFile()` paths inside it for assets that need hosted URLs in previews. References work in both installation modes, including inside objects and arrays.

Use a unique public-relative path scoped to your Element. Every reference must match a declared asset path. Assets can also contain [base64 data](/docs/studio-protocol/create-element-payload#assets) instead of a download URL. To keep an asset remote after installation, leave its URL in `initialProps` without an installation override.

Before submitting, preview the Element, install it into Studio, and confirm the asset appears in the public directory and plays or renders from the local file.
