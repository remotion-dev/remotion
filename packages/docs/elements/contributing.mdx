...
- [Include assets](#include-assets) that should be copied into the user's project. Use stable remote URLs for previewing them and Google Fonts for non-websafe fonts.
...
Accept each asset URL as a component prop, such as `audioSrc`. Use a hosted URL in `initialProps` so the gallery and preview renders work without a matching public-folder structure:
...
Use a unique public-relative path scoped to your Element. Every reference must match a declared asset path. Assets can also contain [base64 data](/docs/studio-protocol/create-element-payload#assets) instead of a download URL; supply a usable preview URL, such as a data URL, in `initialProps`. To keep an asset remote after installation, leave its URL in `initialProps` without an installation override.
