...
Copy the rendered preview assets to `packages/docs/static/elements` and commit them unless you upload them to `remotion.media` and update the preview URLs. Preview assets let reviewers see the Element in the pull request deployment. After approval, a repository maintainer can upload committed preview assets to static storage. Keep “Allow edits from maintainers” enabled on the pull request.
