...
Follow the [starter props guidance](/docs/studio-protocol/component-library-integration#choose-starter-props) for either installation mode. Put starter values in `initial-props.ts`, type them with `satisfies ComponentProps<typeof YourComponent>`, and register them as `initialProps` in `element-definitions.ts`.
