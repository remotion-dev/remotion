...
Copy the rendered preview assets to `packages/docs/static/elements` and commit them. Preview assets let reviewers see the Element in the pull request deployment. After approval, a repository maintainer uploads them to static storage. Keep “Allow edits from maintainers” enabled on the pull request.
