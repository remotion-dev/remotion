...
If the Element needs starter props, put them in `initial-props.ts`, type them with `satisfies ComponentProps<typeof YourComponent>`, and register them as [`initialProps`](/docs/studio-protocol/create-element-payload#initialprops) in `element-definitions.ts`. For `component-owned-sequence`, this can provide defaults for Inspector controls. Otherwise, users can edit the installed source directly.

### Include assets

Follow the [asset integration guidance](/docs/studio-protocol/component-library-integration#preview-installed-assets). Installed assets are separate from the PNG and MP4 gallery previews in [Prepare previews](#2-prepare-previews).

In `element-definitions.ts`, declare `assets` and put [`staticFileRef()`](/docs/studio-protocol/static-file-ref) references in `installationProps`. Keep preview URLs in `initialProps`.

`installationProps` shallowly overrides `initialProps` only for installation; an overridden object or array replaces that entire prop. For Elements without installed assets, use `assets: []` and `installationProps: null`.
