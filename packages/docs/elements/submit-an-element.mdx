...
- An entry in `packages/docs/src/components/Elements/element-registry.ts`
...
To give an Element a fixed size, set `elementWidth` and `elementHeight` in its registry entry. Use `null` for both values if the Element should inherit the full composition dimensions.

```tsx title="element-registry.ts"
...
Use `previewPadding` in the registry entry if the docs preview needs more breathing room without changing the Element's drag dimensions or adding source padding.
...
Set an intentional `posterFrame` in the registry entry. It should show the Element in a representative, visible state.
...
- The Element folder and registry entry
