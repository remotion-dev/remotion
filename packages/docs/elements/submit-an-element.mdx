...
Do not put the placement wrapper [`<Sequence>`](/docs/sequence) into the Element source file.

A custom component using [`Interactive.withSchema()`](/docs/interactive-with-schema) may use one internal `<Sequence layout="none">` to forward `controls` and `outlineRef`. Do not use this Sequence for placement, dimensions, or timing.
