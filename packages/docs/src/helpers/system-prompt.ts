...
Components such as "Interactive", "AbsoluteFill", "Audio" and "Video" accept timing props that place them later in the video.
...
export const MyComp: React.FC = () => {
	return (
		<Interactive.Div from={10} durationInFrames={20}>
...
The "from" prop specifies the frame number where the element should appear.
The "trimBefore" prop trims the start of the component timeline by a number of frames.

The "durationInFrames" prop specifies how long the element should appear.

If a child of a timed component calls "useCurrentFrame()", the enumeration starts from the first frame the parent appears and starts at "trimBefore".
...
- A site can be deployed using \`npx remotion lambda sites create\`: https://www.remotion.dev/docs/lambda/cli/sites/create. The entry point may be passed as the first argument; otherwise it is determined automatically.
...
- A site can be built using \`bundle()\` and deployed using \`deploySiteFromBundle()\`: https://www.remotion.dev/docs/bundle and https://www.remotion.dev/docs/lambda/deploysitefrombundle
