...
<SuggestedPrompt prompt="/remotion-markup Make a new composition with a map. Zoom out from Los Angeles while keeping it in focus, then animate a line from Los Angeles to New York and make the camera follow it." />
...
To serve the worker locally, copy the worker and its imported modules into your [public directory](/docs/staticfile) and pass the worker URL to `setWorkerUrl()`.
