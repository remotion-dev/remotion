...
This keeps the shader deterministic, reusable and editable in Remotion Studio through an [`InteractivitySchema`](/docs/interactivity-schema).
...
`createEffect()` adds the returned factory to the same effect system used by [`@remotion/effects`](/docs/effects/api).
The `schema` fields such as `speed`, `scale` and `warp` can be edited in Studio when the effect is attached to a component that supports effects.

Use a hidden field for animated values such as `time` if the value should be passed by code but not shown as an editable control.
