...
The following component renders an animated shader on top of a transparent [`<Solid>`](/docs/solid).
...
Use [`<Solid>`](/docs/solid) for procedural shader backgrounds. Use `<HtmlInCanvas>` when you need to capture DOM content and run shader effects over its pixels.
