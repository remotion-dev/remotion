---
image: /generated/articles-docs-integration-frameworks-vue-js.png
id: vue-js
sidebar_label: 'VueJS'
title: 'VueJS'
---
...
# Using Remotion with VueJS

This guide explains how to integrate **Remotion** into a **VueJS** project.
...
```
:::note
The "jsxImportSource": "" setting is also recommended to prevent build issues.
:::

## Configure Vite for React

To enable React support in Vite, update the `vite.config.ts` file by setting adding react to the plugins.
...
```ts title="PlayerViewWrapper.vue"
<script lang="ts" setup>
    ...
    watch(() => playerRef, (newData, oldData) => {
...
You're no able to use the [`API`](/docs/player/player) of the player via `this.playerRef()`

## Configure TailwindCSS (optional)

If your project uses **TailwindCSS**, ensure that it applies styles to `.tsx` files by updating the `tailwind.config.js` file as follows:

```js title="tailwind.config.js"
module.exports = {
  content: [
    "./index.html",
    "./src/**/*.{vue,tsx}",,  // Add .tsx files to Tailwind content paths
    // other paths
  ],
...
You should also install the following package:
