...
### `effects?`<AvailableFrom v="4.0.468" />

Apply [effects](/docs/effects/api) to the image.

When `effects` is omitted or an empty array, `<Img>` renders a native `<img>` element. When at least one effect is passed, `<Img>` renders through [`<CanvasImage>`](/docs/canvasimage).
