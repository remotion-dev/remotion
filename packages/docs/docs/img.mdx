...
### `fit?`<AvailableFrom v="4.0.468" />

Controls how the image is drawn into the canvas when [`effects`](#effects) is a non-empty array.

Can be `"fill"`, `"contain"` or `"cover"`. Default: `"fill"`.
