...
Some runtimes ship only the essential codecs to stay lean, so `AV1` is not available on Remotion Lambda or Linux ARM64 GNU. See the [Encoding guide](/docs/encoding) for details.
