...
## `setDefaultTerminal()`<AvailableFrom v="4.0.508" />
...
Available terminals: `terminal`, `iterm2`, `ghostty`, `warp`, `wezterm`, `alacritty`, `windows-terminal`, and `gnome-terminal`.

Only installed terminals for the current operating system are shown in Remotion Studio. The [`--terminal`](/docs/cli/studio#--terminal) command line flag takes precedence over this option.
