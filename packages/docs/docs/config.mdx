...
Adds a third-party Element library to Remotion Studio from an externally hosted Element catalog.
...
Call `addElementLibrary()` once for each third-party Element library. The `displayName` property is optional. If it is omitted, the URL host and path are shown instead.
...
Registering the catalog does not install an individual Element's source code or dependencies. The URL must be an absolute HTTP or HTTPS URL. Remotion Elements remains available without adding it to the configuration.
