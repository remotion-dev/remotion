...
Remotion Lambda ships with the essential codecs to keep the runtime lean, so `AV1` is not available there. See the [Encoding guide](/docs/encoding) for details.
