...
Set a custom port for preview server. 

```tsx
Config.Bundling.setPreviewServerPort(3003);
```

The [command line flag](/docs/cli) `--preview-server-port` will take precedence over this option.
...
The [command line flag](/docs/cli) `--env-file` will take precedence over this option.
