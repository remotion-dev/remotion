...
Adds a third-party Element Library to Remotion Studio from an externally hosted URL.
...
Registering the Element Library does not install an individual Element's source code or dependencies. The URL must be an absolute HTTP or HTTPS URL. Remotion Elements remains available without adding it to the configuration.
