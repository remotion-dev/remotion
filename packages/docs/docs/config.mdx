...
## `setStudioCanvasCaptureEnabled()`<AvailableFrom v="4.0.475" />

<Options id="studio-canvas-capture" />

```ts twoslash title="remotion.config.ts"
import {Config} from '@remotion/cli/config';
// ---cut---
Config.setStudioCanvasCaptureEnabled(true);
```

The [command line flag](/docs/cli/studio#--studio-canvas-capture) `--studio-canvas-capture` will take precedence over this option.
