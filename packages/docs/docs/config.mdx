...
Call `addElementLibrary()` once for each external library. When at least one is configured, Browse Elements shows a dropdown containing Remotion Elements and every configured library. All libraries open in an embedded Studio modal.
