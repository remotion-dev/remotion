...
Some Editor Starter features require backend endpoints.
...
Triggers a render on Remotion Lambda. This route is only used when rendering with Remotion Lambda.
...
Gets the current progress of a render on Remotion Lambda. This route is only used when rendering with Remotion Lambda.
