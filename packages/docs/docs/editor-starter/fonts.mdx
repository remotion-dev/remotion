...
Because of this, font metadata is being stored in the backend and only the metadata of the fonts that are required are loaded to the client. This is the reason for the [`GET /api/fonts/:name`](/docs/editor-starter/backend-routes#get-apifontsname) endpoint.
