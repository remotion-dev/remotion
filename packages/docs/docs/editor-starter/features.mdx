...
Enables UI for rendering the current composition using [Remotion Lambda](/docs/editor-starter/rendering) or [client-side rendering](/docs/editor-starter/client-side-rendering). Shows render progress and allows download of the final rendered video.

### Client-side rendering

Feature flag: [`ENABLE_CLIENT_SIDE_RENDERING`](https://github.com/remotion-dev/editor-starter/blob/main/src/editor/flags.ts)

Renders videos in the browser instead of using Remotion Lambda.
Requires `FEATURE_NEW_MEDIA_TAGS` to also be enabled.
