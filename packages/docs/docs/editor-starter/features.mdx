...
Enables snapping behavior on the timeline, allowing items to automatically align with edges of other items, the playhead, or grid positions when dragging. This makes it easier to precisely position items and create seamless transitions. Enabled by default.
...
Allows toggling timeline snapping on/off using a keyboard shortcut. When enabled, you can temporarily disable snapping while dragging items. Enabled by default. Requires [`FEATURE_TIMELINE_SNAPPING`](https://github.com/search?q=repo%3Aremotion-dev%2Feditor-starter%20FEATURE_TIMELINE_SNAPPING&type=code) to be enabled.
