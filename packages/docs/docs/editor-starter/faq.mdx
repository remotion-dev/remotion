...
In short: Teams and companies with a headcount of 4 or more need to obtain a [Remotion Company License](/docs/license/pricing) - read the license for more information.
...
The Editor Starter supports [client-side rendering using WebCodecs](/docs/editor-starter/client-side-rendering), but it is disabled by default.

By default, rendering happens using [Remotion Lambda](/docs/editor-starter/rendering).

## Do I have to use Remotion Lambda?

No. You can [enable client-side rendering](/docs/editor-starter/client-side-rendering) to render on the user's device.

You can also modify the architecture to render on a long-running server using the Remotion [server-side rendering](/docs/ssr) APIs:
...
The state is stored as JSON and fed as [input props](/docs/terminology/input-props) to the [`<Player />`](/docs/player) and either [`renderMediaOnLambda()`](/docs/lambda/rendermediaonlambda) or [`renderMediaOnWeb()`](/docs/web-renderer/render-media-on-web).
...
## Can I include the Editor Starter in an open source project?

No. The Editor Starter source code may not be redistributed, including as part of an open source project - this would create a leak of the source code.

You may use individual components, but you may not incorporate the entire Editor Starter codebase into a project that is publicly available.
