...
Animate the annotation separately from the text so the reader can first parse the words, then notice the emphasis.

The examples below match the rough notation compositions in `packages/example/src/RoughNotation`.

## Highlight

Use [`<AnnotationBehind>`](/docs/rough-notation/annotation-behind) from [`@remotion/rough-notation`](/docs/rough-notation/api) for a hand-drawn highlight behind text.
