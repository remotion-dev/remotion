...
This package allows holders of the [Company License](/docs/pricing) to send events to Remotion to track the usage of renders.  
...
From Remotion 5.0, opt-out is possible under an [Enterprise License](/docs/pricing) agreement if monthly usage reports are provided instead.  
...
- **Custom pricing for high render volume**: For especially high render volumes where standard per-render pricing no longer fits, we offer custom pricing that requires invoicing (i.e. custom billing).
- **Vendor onboarding and compliance**: We complete vendor onboarding and comply with your vendor policies.
- **Closer collaboration and highest priority support**: Work directly with the Remotion team through a private Slack or Discord channel, with the highest priority support.
- **Feature request consideration**: Your feature requests are given consideration for inclusion in the product roadmap.
