...
This package allows holders of the [Company License](https://remotion.pro/license) to send events to Remotion to track the usage of cloud renders.
...
In Remotion 5.0, it will be mandatory for customers to report renders using `@remotion/licensing`.

**For [`@remotion/lambda`](/docs/lambda)**: This package is a dependency and renders will trigger a call to [`registerUsagePoint()`](/docs/licensing/register-usage-point) if you pass the `apiKey` option.
**For [`@remotion/renderer`](/docs/renderer)**: You can pass the `apiKey` option to [`renderMedia()`](/docs/renderer/render-media#apikey) and [`renderStill()`](/docs/renderer/render-still#apikey) to track local render usage.
