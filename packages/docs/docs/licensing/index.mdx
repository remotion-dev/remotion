...
We plan to introduce a retention policy that automatically deletes telemetry data after a defined period. Before deletion, the data will be anonymized so that no identification of an individual end user is possible. This has not been implemented yet, but is on our roadmap for the future.
