...
<Step>1</Step> Commit your repository to GitHub

<Step>2</Step> Add the following code to `.github/workflows/deploy-studio.yml`

```yaml twoslash title=".github/workflows/deploy-studio.yml"
...
jobs:
  render:
    ...
    steps:
      ...
      - name: Bundle studio
        run: bunx remotion bundle --public-path="./"
...
<Step>3</Step> Go to `github.com/username/repo/settings/pages`

<Step>4</Step> In the Branch section, select the `gh-pages` branch and click save
