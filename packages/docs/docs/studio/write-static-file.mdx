...
export const WriteStaticFileComp: React.FC = () => {
  const saveFile = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
...
.
<br />
<Step>3</Step> It's not allowed to write outside the
<a href="/docs/terminology/public-dir">
  <code>public</code> directory
</a>
.<br />
<Step>4</Step> To write into subfolders, use forward slashes <code>/</code> even on Windows.
