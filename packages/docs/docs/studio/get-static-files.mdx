...
:::note
In the Player or when called directly in Node.js or Bun, this API returns an empty array. It is available in Remotion Studio and inside compositions during [server-side rendering](/docs/ssr).
:::
...
<CompatibilityTable chrome firefox safari nodejs="" bun="" serverlessFunctions="" clientSideRendering={false} serverSideRendering player="Returns an empty array" studio hideServers />
