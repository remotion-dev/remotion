...
The Studio leaves computed values untouched, so their runtime result is preserved.
Inline a computed value only when it should become editable or keyframable in the Studio.
...
Values inherited from an object spread remain computed and render with their runtime value.
Put a property after the spread to make that property editable without inlining the other shared styles.
...
<Interactive.Div
  ...
  style={{
    ...container,
    color: 'white', // Editable because it is declared after the spread
