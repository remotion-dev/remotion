...
To make a caption area adjustable, add `width` and `height` number fields to the component schema and forward them to the [`<Sequence>`](/docs/sequence). These fields define the area available to captions; wrapping and scaling remain component-defined.
