...
If you only need an editable HTML or SVG element, use [`Interactive`](/docs/interactive). For a custom component, wrap it with [`Interactive.withSchema()`](/docs/with-interactivity-schema).

## Create a schema

An [`InteractivitySchema`](/docs/interactivity-schema) describes which props the Studio may edit.

Use [`Interactive.baseSchema`](/docs/interactive#interactivebaseschema) for props inherited from [`<Sequence>`](/docs/sequence), such as `from`, `durationInFrames`, `freeze`, `hidden`, `name` and `showInTimeline`.
...
This example uses the default `<Sequence>` layout so the Studio can draw an outline around the component in the preview. A `<Sequence layout="none">` can still expose timeline controls, but it does not register an outline automatically.
