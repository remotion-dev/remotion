...
Browse Elements includes the first-party [Remotion Elements](/elements). You can add [third-party Element libraries](/elements/libraries) to the library selector for more options.

Adding a library makes its catalog available to browse. Installing an individual Element's source code remains a separate, confirmed action.
