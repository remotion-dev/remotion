...
As a rule, include [`Interactive.baseSchema`](/docs/interactive#interactivebaseschema) in every schema passed to `Interactive.withSchema()` so standard timeline controls remain available.
...
Rendering support depends on the wrapped component. Editing timeline controls requires an editable Remotion Studio session.
