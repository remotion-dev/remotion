...
### `detached?`<AvailableFrom v="4.0.468" />
...
### `vercelBlob?`<AvailableFrom v="4.0.468" />
...
This option is required when [`detached: true`](#detached), because the route that started the render returns before the video exists.

### `detachedSandboxTimeoutInMilliseconds?`<AvailableFrom v="4.0.468" />
