...
Use `outputType` to explicitly select and validate the CSS value type:

```tsx twoslash
import {interpolate} from 'remotion';

const rotation = interpolate(0.5, [0, 1], ['0deg', '90deg'], {
  outputType: 'rotate',
});
```

Supported values are `'scale'`, `'translate'`, `'rotate'`, `'transform-origin'`, and `'font-weight'`.
If omitted, scale, translate, rotate, and transform origin values continue to be inferred.

## Example: Font weights

Font weight keywords require `outputType: 'font-weight'`.
The keywords `normal` and `bold` are normalized to `400` and `700`.
Numeric strings and numbers between `1` and `1000` are also supported.
The result is a number.

```tsx twoslash
import {interpolate} from 'remotion';

const fontWeight = interpolate(0.5, [0, 1], ['normal', 'bold'], {
...
Use `Easing.step1` to switch between strings that cannot be blended. The previous value is held until the next keyframe.

```tsx twoslash title="MyComposition.tsx"
import {Easing, interpolate, useCurrentFrame} from 'remotion';

const frame = useCurrentFrame();
const cursor = interpolate(frame, [0, 100], ['default', 'ne-resize'], {
  easing: Easing.step1,
});
```

Every segment must use `Easing.step1`. If `easing` is an array, each entry must be `Easing.step1`.
...
Explicitly selects the output value syntax.
Accepts `'font-weight'`, `'scale'`, `'translate'`, `'rotate'`, or `'transform-origin'`.
Font weight keywords are only supported when this option is set to `'font-weight'`.
...
const outputType: InterpolateOutputType = 'rotate';
