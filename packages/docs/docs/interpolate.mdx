...
## Example: Scaling feels slow? Use exponential output<AvailableFrom v="4.0.488" />

For `scale` or zoom-like values, additive interpolation can feel like it slows down as the number grows.
Use `output: 'exponential'` to distribute values by equal ratios after easing.

```tsx twoslash title="MyComposition.tsx"
import {Easing, interpolate, useCurrentFrame} from 'remotion';

const frame = useCurrentFrame();
const scale = interpolate(frame, [0, 60], [0.5, 2], {
...
Every output value must be greater than `0` when using `output: 'exponential'`.
...
### output?<AvailableFrom v="4.0.488" />
...
Use `output: 'exponential'` for multiplicative interpolation.
For `[1, 2]` at progress `0.5`, the result is `Math.sqrt(2)`.
This is useful for `scale`, camera zoom, and other values where equal ratios feel more natural than equal deltas.

`easing` still controls time and progress.
`output` controls the value distribution after easing.
All numeric output values must be greater than `0` when using `output: 'exponential'`.
