...
Reference an Element asset in [`initialProps`](/docs/studio-protocol/create-element-payload#initialprops). When the Element is installed, Studio downloads the asset into the user's project's `public/` directory and generates a [`staticFile()`](/docs/staticfile) expression on the component invocation. The Element source is not rewritten.
