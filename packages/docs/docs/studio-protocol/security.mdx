...
An Element contains executable React source code and may declare npm dependencies. Studio requires confirmation before applying an Element delivered through drag-and-drop or an installation request. It also requires confirmation before adding an external Element Library to a project.

## Allowed origins

Any HTTPS website can request an Element installation or request to add an Element Library. HTTP is supported only for local development pages on `localhost` or `127.0.0.1`.
...
An Element Library request made with [`addElementLibraryToStudio()`](/docs/studio-protocol/add-element-library-to-studio) shows the requesting origin, exact Element Library URL, and requested display name. The Element Library is not loaded before confirmation. Confirming persists a [`Config.addElementLibrary()`](/docs/config#addelementlibrary) call in `remotion.config.ts`; it does not install Element source code or dependencies. Individual Element installations still require confirmation.

Declining or closing the Element Library confirmation does not change the config.
