...
Use [`staticFileRef()`](/docs/studio-protocol/static-file-ref) when an asset should use a hosted URL in the Player but become [`staticFile()`](/docs/staticfile) after installation. Put the call directly on the media or interactive layer and declare its public-relative path in the payload's [`assets`](/docs/studio-protocol/create-element-payload#assets).

Assets that should remain remote after installation can use their URL directly. `staticFileRef()` is separate from `initialProps`; it keeps asset references on the editable layers in wrapped Elements.
