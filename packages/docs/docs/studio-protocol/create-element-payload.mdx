...
Set `path` to a forward-slash path relative to the public directory. When the same source runs in a website preview, reference it with [`staticFileRef()`](/docs/studio-protocol/static-file-ref). Studio writes [`staticFile()`](/docs/staticfile) into the installed source. Paths cannot be absolute, contain traversal or platform-unsafe segments, or conflict with another declared asset path.
