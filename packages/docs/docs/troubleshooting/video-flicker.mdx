...
<strong style={{color: 'var(--ifm-color-primary)'}}>Recommended</strong>: This
will become the default behavior in Remotion 5.0
...
<strong>Not recommended</strong>: Memory intensive, and only useful if
prefetched before the Player is mounted, and often misused.
...
<strong>Not recommended</strong>: Same drawbacks as prefetching, but with an
additional performance hit, especially for large assets.
