...
**If you are rendering on Lambda**: Create a bundle during the build step and upload it later using [`deploySiteFromBundle()`](/docs/lambda/deploysitefrombundle). You may also bundle and upload together using [`npx remotion lambda sites create`](/docs/lambda/cli/sites/create) or [`deploySite()`](/docs/lambda/deploysite). Refer to the deployed bundle using the URL returned by the deployment.
