...
For new projects, [`<Video>` from `@remotion/media`](/docs/media/video) is now recommended over [`<OffthreadVideo>`](/docs/offthreadvideo).

See the [comparison of video tags](/docs/videos/video-tags) for the differences between the available video components.
