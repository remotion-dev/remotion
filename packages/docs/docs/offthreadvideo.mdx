...
# &lt;OffthreadVideo&gt;<AvailableFrom v="3.0.11" />
...
Selects this many source frames starting at `trimBefore`. The video occupies `durationInFrames / playbackRate` frames on the parent timeline.
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <OffthreadVideo src={staticFile('video.webm')} trimBefore={60} durationInFrames={60} />
    </AbsoluteFill>
  );
};
```
