...
### `trimLeft?`<AvailableFrom v="4.0.319"/>
...
By passing `trimLeft={60}`, the playback starts immediately, but with the first 2 seconds of the video trimmed away.  
By passing `trimRight={120}`, any video after the 4 second mark in the file will be trimmed away.
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <OffthreadVideo src={staticFile('video.webm')} trimLeft={60} trimRight={120} />
    </AbsoluteFill>
  );
};
```

### `trimRight?`<AvailableFrom v="4.0.319"/>

Removes a portion of the video at the end (right side). See [`trimLeft`](/docs/offthreadvideo#trimleft) for an explanation.

### `startFrom?` <DeprecatedBlock>

:::note Deprecated
This prop is deprecated in favor of [`trimLeft`](#trimleft). It will continue to work but you cannot use it together with the new prop.
:::
...
### `endAt?` <DeprecatedBlock>

:::note Deprecated
This prop is deprecated in favor of [`trimRight`](#trimright). It will continue to work but you cannot use it together with the new prop.
:::
