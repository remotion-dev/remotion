...
This accounts for [`playbackRate`](/docs/sequence#playbackrate) and [`trimBefore`](/docs/sequence#trimbefore). For example, a sequence visible for `40` parent frames with `playbackRate={2}` and `trimBefore={10}` returns `90`: `40 * 2 + 10`. Its children start at local frame `10` and unmount before local frame `90`.
