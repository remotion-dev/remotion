...
Remotion supports 6 video codecs: `h264` (_default_), `h265`, `vp8`, `vp9`, `av1` and `prores`. While H264 will work well in most cases, sometimes it's worth going for a different codec. Refer to the table below to see the advantages and drawbacks of each codec.
...
<table>
  ...
  <tr>
    ...
    <td>macOS, Linux, Windows (NVIDIA GPU)</td>
  </tr>
  <tr>
    <td>
  ...
  <tr>
    <td>AV1*</td>
    <td>.mp4, .webm or .mkv</td>
    <td style={{color: 'green', fontWeight: 'bold'}}>Very small</td>
    <td style={{color: 'red', fontWeight: 'bold'}}>Very slow</td>
    <td>
      <a href="https://caniuse.com/av1" style={{color: 'darkorange'}}>
        Okay
      </a>
    </td>
    <td>No</td>
...
* AV1 is not available on [Remotion Lambda](/docs/lambda) or Linux ARM64 GNU.

AV1 can achieve very small file sizes, but encoding is significantly slower than H.264 and H.265. Some runtimes ship only the essential codecs to stay lean, so `AV1` output is not available on Remotion Lambda or Linux ARM64 GNU.
...
_Applies only to `h264`, `h265`, `vp8`, `vp9` and `av1`._
...
- In SSR, Lambda and Cloud Run APIs: Use the [`videoBitrate`](/docs/renderer/render-media#videobitrate) and [`audioBitrate`](/docs/renderer/render-media#audiobitrate) options.
...
For ProRes, there is no CRF option, but there are profiles which you can set using the [`--prores-profile` flag](/docs/cli/render#--prores-profile), the [`proResProfile`](/docs/renderer/render-media#proresprofile) render option, or the [`setProResProfile()`](/docs/config#setproresprofile) config file option.

See [Rendering ProRes](/docs/prores#choosing-a-prores-profile) for all profile values and alpha channel support.
