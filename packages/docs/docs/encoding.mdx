...
* AV1 is not available on [Remotion Lambda](/docs/lambda).

AV1 can achieve very small file sizes, but encoding is significantly slower than H.264 and H.265. Remotion Lambda ships with the essential codecs to keep the runtime lean, so `AV1` output is not available there.
