...
<CompatibilityTable chrome="AudioData" firefox="AudioData" safari="AudioData" nodejs={false} bun={false} serverlessFunctions={false} clientSideRendering serverSideRendering player studio />

Requires the WebCodecs [`AudioData`](https://developer.mozilla.org/en-US/docs/Web/API/AudioData) API in a secure context. Server-side rendering runs this API inside Chromium.
