...
Bonus confusion: Handwritten assembly is not the same as WebAssembly
 ...
 Think of WebAssembly as another compilation target alongside x86, ARM etc., rather than associate it with Assembly languages.
</details>
...
But WebGPU is the name of a specific API inside the browser that is mostly used for graphics or machine learning stuff.
...
It works because JavaScript only deals with the cheap operations, and WebCodecs takes care of the computationally expensive stuff.

A library like [Mediabunny](https://mediabunny.dev) mainly reads and writes data from a file (demuxing and muxing), but the expensive stuff is to decompress and compress the media data (decoding and encoding).

A multimedia library that does not use WebCodecs and uses code compiled to WebAssembly to decode and encode cannot take advantage of many of the hardware-specific optimisations that are possible on a computer, for example using CPU-specific instructions, or using the GPU.

## WebCodecs can not be used on React Native or Node.js
