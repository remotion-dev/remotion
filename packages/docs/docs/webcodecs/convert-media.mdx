...
<CompatibilityTable chrome="Supported codecs" firefox="Supported codecs" safari="Supported codecs" nodejs="Copy only" bun="Copy only" serverlessFunctions="Copy only" clientSideRendering="Supported codecs" serverSideRendering="Supported codecs" player="Supported codecs" studio="Supported codecs" />

Re-encoding requires [WebCodecs](https://developer.mozilla.org/en-US/docs/Web/API/WebCodecs_API) in a secure context and support for the input and output codecs. Without WebCodecs, only compatible tracks can be copied or dropped; use a writer supported by the runtime.
