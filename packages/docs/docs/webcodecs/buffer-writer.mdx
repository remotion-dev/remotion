...
Requires resizable `ArrayBuffer` and `File` APIs, as available in the [Media Parser runtimes](/docs/media-parser/runtime-support). The writer itself does not require WebCodecs.
