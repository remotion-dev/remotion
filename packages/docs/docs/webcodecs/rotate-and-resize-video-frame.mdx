...
<CompatibilityTable chrome="VideoFrame" firefox="VideoFrame" safari="VideoFrame" nodejs={false} bun={false} serverlessFunctions={false} clientSideRendering serverSideRendering player studio />

Requires WebCodecs [`VideoFrame`](https://developer.mozilla.org/en-US/docs/Web/API/VideoFrame) and a 2D `OffscreenCanvas` context. Server-side rendering runs this API inside Chromium.
