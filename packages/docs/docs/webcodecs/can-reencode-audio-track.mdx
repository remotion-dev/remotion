...
This capability check can run without WebCodecs. A `true` result for PCM-to-WAV conversion still requires `AudioData` and `EncodedAudioChunk` to perform the conversion. Other supported codecs vary by browser and operating system.
