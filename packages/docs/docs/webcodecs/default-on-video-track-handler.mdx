...
The handler can run without WebCodecs. Re-encoding is selected only when the runtime supports the required decoder and encoder; otherwise it copies, drops or rejects the track.
