...
The handler can run without WebCodecs. Performing a selected re-encoding still requires the relevant WebCodecs APIs, including `AudioData` and `EncodedAudioChunk` for PCM audio.
