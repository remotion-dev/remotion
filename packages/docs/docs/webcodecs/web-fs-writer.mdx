...
<CompatibilityTable chrome="Feature check" firefox="Feature check" safari="Feature check" nodejs={false} bun={false} serverlessFunctions={false} clientSideRendering="Feature check" serverSideRendering="Feature check" player="Feature check" studio="Feature check" />

Requires access to the origin-private filesystem and writable file streams. Call [`canUseWebFsWriter()`](#canusewebfswriter) before use; availability also depends on the page origin and browser storage settings.
