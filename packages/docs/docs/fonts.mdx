...
Fonts loaded through CSS or the browser's `FontFace` API are not embedded into SVG images. See [client-side rendering limitations](/docs/client-side-rendering/limitations#web-fonts-inside-svg) for more details.
