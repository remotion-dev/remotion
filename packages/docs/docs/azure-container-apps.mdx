...
```bash title="set evironment variables"
...
`Azure CLI` will deploy the image to `Azure Container Apps` when completed will provide a url which accepts `HTTP` request.
...
Send a GET request to the get the render status, replace `YOUR_SERVICE_URL` with the URL from the deployment output and the `jobId` from the render request output.
...
If `status` is `completed`, combine `YOUR_SERVICE_URL` and the `video_url` response to generate the download link (`https://remotion-gpu.grayocean-24741fc9.australiaeast.azurecontainerapps.io/renders/955338f6-2607-48bd-bedb-6d4c98f7b4dc.mp4`).  
...
```
This will delete all the resource inside the resource group including application container.
