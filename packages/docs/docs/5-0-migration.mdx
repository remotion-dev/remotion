...
- **For customers, the license is now tied to our updated [terms and conditions](/docs/terms)**.
...
### `renderMedia()` and `renderStill()` warn if `licenseKey` is not set

In Remotion 5.0, [`renderMedia()`](/docs/renderer/render-media) and [`renderStill()`](/docs/renderer/render-still) log a warning if the optional `licenseKey` option is omitted or set to `null`. No telemetry event is sent if no license key is provided.

If you qualify for the [Free License](/docs/license), pass `licenseKey: "free-license"` to acknowledge the license and disable the warning. This value does not send a telemetry event either.
