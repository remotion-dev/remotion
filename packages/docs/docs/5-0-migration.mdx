...
## `<Player>` does not create shared audio tags by default

The default [`numberOfSharedAudioTags`](/docs/player/player#numberofsharedaudiotags) value of the [`<Player>`](/docs/player/player) has changed from `5` to `0`.

**Required action**: If you rely on shared audio tags to work around browser autoplay policies, set `numberOfSharedAudioTags={5}` or another value that covers the maximum number of [`<Html5Audio>`](/docs/html5-audio) tags playing at the same time.
