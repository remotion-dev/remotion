...
### `renderMedia()` warns if `licenseKey` is not set

In Remotion 5.0, [`renderMedia()`](/docs/renderer/render-media) logs a warning if the optional `licenseKey` option is omitted or set to `null`. No telemetry event is sent if no license key is provided.
