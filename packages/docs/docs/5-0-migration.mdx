...
## `--disable-headless` and `headless: false` are removed

Since the introduction of [Chrome Headless Shell](/docs/miscellaneous/chrome-headless-shell), it is not possible to disable headless mode anymore.  
This flag was a no-op and has now been removed.
