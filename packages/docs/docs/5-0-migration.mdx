...
## `deploySite()` is deprecated

[`deploySite()`](/docs/lambda/deploysite) continues to bundle and deploy a project, but is deprecated because it combines both operations. Existing calls continue to work, so no migration is required.

The recommended workflow is to build with [`bundle()`](/docs/bundle) and deploy its output with [`deploySiteFromBundle()`](/docs/lambda/deploysitefrombundle):
