...
## `deploySite()` no longer bundles projects

[`deploySite()`](/docs/lambda/deploysite) now requires a pre-built `bundleDir`. Building and deployment are separate operations.

```ts twoslash title="deploy.ts"
import {bundle} from '@remotion/bundler';
import {deploySite} from '@remotion/lambda';

const bundleDir = await bundle({entryPoint: './src/index.ts'});
...
Pass build-only options such as bundler overrides, `publicDir`, `rootDir`, and `gitSource` to [`bundle()`](/docs/bundle). Replace the `onBundleProgress` callback with [`bundle()`'s `onProgress`](/docs/bundle#onprogress) callback.

Existing calls to [`deploySiteFromBundle()`](/docs/lambda/deploysitefrombundle) continue to work.

**Required action**: Call [`bundle()`](/docs/bundle) separately and replace `entryPoint` with `bundleDir` when calling [`deploySite()`](/docs/lambda/deploysite).

The Lambda CLI also expects a bundle directory:
