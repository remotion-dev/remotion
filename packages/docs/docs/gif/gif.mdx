...
Options that are passed to the internal [`fetch()`](https://developer.mozilla.org/en-US/docs/Web/API/fetch) call when loading the GIF. This can be used to pass credentials or headers for authenticated GIFs.
