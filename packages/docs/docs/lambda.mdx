...
- Remotion Lambda ships with the essential codecs to keep the runtime lean. The same lean binary set is also used on Linux ARM64 GNU, so `AV1` output is not available there either.
