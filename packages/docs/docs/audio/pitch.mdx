---
image: /generated/articles-docs-audio-pitch.png
title: Controlling Pitch
sidebar_label: Controlling Pitch
id: pitch
crumb: 'Audio'
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

:::warning
Pitch correction is currently only applied during rendering.
:::

You can use the [`toneFrequency`](/docs/audio#tonefrequency) prop to control the pitch of the audio during rendering.

Values between `0.01` and `2` are accepted, where `1` represents the original pitch. Values less than `1` will decrease the pitch, while values greater than `1` will increase it.

A [`toneFrequency`](/docs/audio#tonefrequency) of 0.5 would lower the pitch by half, and a [`toneFrequency`](/docs/audio#tonefrequency) of `1.5` would increase the pitch by 50%.

```tsx twoslash {7} title="MyComp.tsx"
import {Audio, staticFile, AbsoluteFill} from 'remotion';

export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <div>Hello World!</div>
      <Audio src={staticFile('audio.mp3')} toneFrequency={0.8} />
    </AbsoluteFill>
  );
};
```
