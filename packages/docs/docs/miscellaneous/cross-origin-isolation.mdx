...
It can also improve performance for packages that can use shared-memory multithreading. For example, [`@mediabunny/prores`](/docs/mediabunny/formats#prores) uses a faster ProRes decoding path when cross-origin isolation is enabled, and falls back to a slower algorithm otherwise.
