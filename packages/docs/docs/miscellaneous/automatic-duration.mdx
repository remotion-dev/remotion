...
- **Timing props and `<Series>` can be dynamic too**
  While it is true that many compositions use timing props or [`<Series>`](/docs/series) from which the duration could be derived in many cases, it is also not a bulletproof solution.
  ...
  A practical usecase of this is [to change the speed of a video over time](/docs/videos/accelerated-video).
