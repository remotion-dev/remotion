...
Chrome used to ship with a `--headless` flag, which we would use.

As of Chrome 123, the headless mode is being split up into:

- `--headless=old`, which is ideal for screenshotting (Remotion needs this)
- `--headless=new`, which is for browser testing
