...
**Output formats**: Videos can be rendered to `H.264` (MP4), `H.265` (HEVC), `VP8` and `VP9` (WebM), `AV1`, `ProRes` as well as GIFs. `AV1` is not available on Remotion Lambda or Linux ARM64 GNU because those runtimes ship only the essential codecs to stay lean. See the [Encoding guide](/docs/encoding) for more detail.
