...
**Output formats**: Videos can be rendered to `H.264` (MP4), `H.265` (HEVC), `VP8` and `VP9` (WebM), `AV1`, `ProRes` as well as GIFs. Remotion Lambda ships with the essential codecs to keep the runtime lean, so `AV1` is not available there. See the [Encoding guide](/docs/encoding) for more detail.
