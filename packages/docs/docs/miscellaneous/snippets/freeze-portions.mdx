...
import {Freeze, Interactive, useCurrentFrame} from 'remotion';

const CurrentFrame: React.FC = () => {
  return useCurrentFrame();
};

export const Counter: React.FC<{from: number}> = ({from}) => {
  return (
    <Interactive.Div
      from={from}
      className="flex h-full justify-center items-center text-6xl"
    >
      <CurrentFrame />
    </Interactive.Div>
  );
