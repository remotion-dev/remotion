...
export const Counter: React.FC = () => {
  return <div className="flex h-full justify-center items-center text-6xl">{useCurrentFrame()}</div>;
};
...
export const FreezePortion: React.FC = () => {
  ...
  const nextFreeze = freezes.find((freeze) => frame < freeze.start + freeze.durationInFrames);
  const activeFreeze = freezes.find((freeze) => frame >= freeze.start && frame < freeze.start + freeze.durationInFrames);
  ...
  const from = useMemo(() => {
    ...
    return freezes[freezes.length - 1].from + freezes[freezes.length - 1].durationInFrames;
