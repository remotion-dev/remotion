...
export const AcceleratedVideo: React.FC = () => {
  ...
  return (
    <Sequence from={frame}>
      <OffthreadVideo startFrom={Math.round(remappedFrame)} playbackRate={speedFunction(frame)} src="https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4#disable" />
    </Sequence>
  );
};
```
