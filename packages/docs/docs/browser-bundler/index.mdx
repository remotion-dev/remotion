...
The [Vibe Code template](/templates/vibe-code) is a complete motion graphics editor built on this package together with [`@remotion/canvas`](/docs/canvas), [`@remotion/codemods`](/docs/codemods) and [`@remotion/web-renderer`](/docs/web-renderer): Fast Refresh in an isolated preview, a timeline, an inspector that writes edits back to the source, and rendering in the browser.

```bash
npx create-video@latest --template vibe-code
```
