...
For longer Vercel Sandbox renders, start [`renderMediaOnVercel()`](/docs/vercel/render-media-on-vercel) with `detached: true` and poll it with [`getRenderProgress()`](/docs/vercel/get-render-progress).

The sandbox can keep rendering after the function has returned, up to the maximum Vercel Sandbox timeout of your plan.
...
Vercel Sandbox supports polling progress for detached renders.
Rendering with Vercel Sandbox or the Node.JS APIs does not come with the other features, but you may build them based on top of the low-level APIs that are provided.

`AV1` output is not available on Remotion Lambda or Linux ARM64 GNU because those runtimes ship only the essential codecs to stay lean.
