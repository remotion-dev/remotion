...
Codec support also differs between environments. For example, `AV1` output is available for regular renders, but Remotion Lambda ships with the essential codecs to keep the runtime lean, so `AV1` is not available there.
