...
Since <AvailableFrom v="4.0.516" inline />, the Studio [infers basic Props editor controls from the values in `defaultProps`](/docs/default-props-inference). A [`schema`](#schema) is not required.
...
Pass a top-level [`z.object()`](https://zod.dev/?id=objects) schema which your default props must satisfy. The schema validates the props and customizes their [visual editing controls](/docs/visual-editing).

If omitted, the Studio [infers basic controls from `defaultProps`](/docs/default-props-inference). An explicit schema takes precedence over the inferred controls. See: [Define a schema](/docs/schemas).
