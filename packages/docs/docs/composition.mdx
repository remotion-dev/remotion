...
Since <AvailableFrom v="4.0.516" inline />, the Studio infers basic Props editor controls from the values in `defaultProps`. A [`schema`](#schema) is not required.
...
If omitted, the Studio infers basic controls from `defaultProps`. An explicit schema takes precedence over the inferred controls. See: [Define a schema](/docs/schemas).
