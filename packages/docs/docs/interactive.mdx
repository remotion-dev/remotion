...
`durationInFrames`, `from`, `trimBefore`, `trimAfter`, `playbackRate`, `loop`, `freeze`, `hidden`, `name` and `showInTimeline`.

`trimAfter` and `loop` are included since <AvailableFrom v="4.0.530" inline />.

Use it for components that render a `<Sequence>` internally and forward these props to it.
...
The same controls as [`Interactive.baseSchema`](#interactivebaseschema) without `loop`.

Use it for components whose output does not change with the frame, such as a static image or shape, so that the Studio does not show a `Loop` toggle that has no effect.
