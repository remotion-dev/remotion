...
Pass an array of ranges to render discontinuous sections. The ranges must be ordered and non-overlapping. An open-ended range may only appear last.<AvailableFrom v="4.0.502" inline />

```ts twoslash title="render-frames.ts"
import {renderFrames} from '@remotion/renderer';
declare const options: Parameters<typeof renderFrames>[0];
// ---cut---
await renderFrames({
	...options,
  frameRange: [
...
Cannot be combined with [`frames`](#frames) or [`everyNthFrame`](#everynthframe).

### `frames?`<AvailableFrom v="4.0.502" />

An array of frame numbers to render. The frames may be passed in any order and are rendered in ascending order. The source frame number is preserved in the output filename.
...
Cannot be combined with [`frameRange`](#framerange) or [`everyNthFrame`](#everynthframe). Use this option to sample stills; its output cannot be passed to [`stitchFramesToVideo()`](/docs/renderer/stitch-frames-to-video).
...
Cannot be combined with [`frames`](#frames).
