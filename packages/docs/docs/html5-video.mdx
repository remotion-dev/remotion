...
By wrapping the video in a [`<Sequence>`](/docs/sequence) with `durationInFrames={60}`, it plays for 60 frames.
  ...
  return (
    <AbsoluteFill>
      <Sequence durationInFrames={60}>
        <Html5Video src={staticFile('video.webm')} trimBefore={60} />
      </Sequence>
    </AbsoluteFill>
  );
};
...
:::info Deprecated
Wrap the component in a [`<Sequence>`](/docs/sequence) and use [`durationInFrames`](/docs/sequence#durationinframes) instead. See [PR #11685](https://github.com/remotion-dev/remotion/pull/11685).
:::
