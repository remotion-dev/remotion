...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <Video volume={(f) => interpolate(f, [0, 100], [0, 1], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'})} src={staticFile('video.webm')} />
    </AbsoluteFill>
  );
};
```
...
[`<OffthreadVideo>`](/docs/offthreadvideo) is a Rust-based alternative to `<Video>`.  
[`@remotion/media`](/docs/media/video) is an experimental component that will replace `<Video>` at some point.
