...
Selects this many source frames starting at `trimBefore`. The video occupies `durationInFrames / playbackRate` frames on the parent timeline, or uses this range as its loop when `loop` is enabled.
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <Html5Video src={staticFile('video.webm')} trimBefore={60} durationInFrames={60} />
    </AbsoluteFill>
  );
};
```
...
- [`Change the speed of a video over time`](/docs/videos/accelerated-video)
