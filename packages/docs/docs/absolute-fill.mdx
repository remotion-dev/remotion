...
Pass these props directly to `<AbsoluteFill>` instead of adding an outer `<Sequence>` just to name or time the layer.
Use an outer `<Sequence>` when grouping multiple children or using features such as [premounting](/docs/sequence#premountfor) or [width and height overrides](/docs/sequence#width).
When nesting them, timing [cascades](/docs/sequence#cascading) like nested sequences.
...
export const MyComp: React.FC = () => {
  return (
    <AbsoluteFill name="Background" from={30} durationInFrames={90}>
