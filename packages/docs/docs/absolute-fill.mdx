...
## Sequencing<AvailableFrom v="4.0.501" />

`<AbsoluteFill>` is registered as a layer in the [Remotion Studio](/docs/studio) timeline and supports timing props.

### from?

The frame at which the layer starts. Default: `0`.

### durationInFrames?

The duration of the layer. Default: `Infinity`.

### trimBefore?

Shifts the local timeline of the children without changing when the layer starts. Default: `0`.

### freeze?

Pins the children to the specified local frame. By default, the children are not frozen.

### hidden?

If `true`, the layer and its children are not rendered. Default: `false`.

### name?

The label shown in the Studio timeline. Default: `"<AbsoluteFill>"`.

### showInTimeline?

If `false`, the layer is omitted from the Studio timeline. Default: `true`.

## Visual editing<AvailableFrom v="4.0.501" />

Inline `style` values such as `backgroundColor`, `opacity`, borders, typography and transforms can be edited in the Studio.

Use `<AbsoluteFill>` for layers you want to see in the timeline. Use a regular `<div>` for structural layout that should not create a timeline item.
