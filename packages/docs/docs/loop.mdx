...
[Many components](/docs/timing) accept a [`loop`](/docs/timing#loop-repeat-an-item) nowadays.
...
How many frames of the child timeline one iteration of the loop should contain. An iteration occupies `durationInFrames / playbackRate` frames in the parent timeline.

### `playbackRate?`<AvailableFrom v="4.0.528" />

Changes the speed of the children within each iteration. Defaults to `1` and must remain constant across frames. Each iteration lasts `durationInFrames / playbackRate` frames in the parent timeline. See [`playbackRate`](/docs/sequence#playbackrate) on [`<Sequence>`](/docs/sequence).
