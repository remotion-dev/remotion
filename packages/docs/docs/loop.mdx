...
### Inherited props<AvailableFrom v="4.0.528" />

`<Loop>` inherits [`premountFor`](/docs/sequence#premountfor), [`postmountFor`](/docs/sequence#postmountfor), [`styleWhilePremounted`](/docs/sequence#stylewhilepremounted) and [`styleWhilePostmounted`](/docs/sequence#stylewhilepostmounted) from [`<Sequence>`](/docs/sequence). These props require `layout="absolute-fill"` (the default). Postmounting the end of the loop requires a finite `times`.

## Relation to `<Sequence loop>`<AvailableFrom v="4.0.530" />

[`<Sequence>`](/docs/sequence#loop) and other timed components accept a [`loop`](/docs/timing#loop-repeat-an-item) prop that repeats the range selected by `trimBefore` and `trimAfter`.

`<Loop>` measures its period in the parent timeline and counts iterations with `times`:
...
const WithSequence = () => (
  <Sequence trimAfter={50} durationInFrames={150} loop>
    <BlueSquare />
  </Sequence>
);
```

Both render the same frames. Use `<Sequence loop>` if the content should start at a `trimBefore` offset or play at a `playbackRate`.
