...
Controls whether rendering intentionally pauses between expensive render phases so the page stays responsive.
...
- `"low"`: Pause after at least `100ms` of rendering work.
- `"medium"`: Pause after at least `33ms` of rendering work.
- `"high"`: Pause after at least `16ms` of rendering work.
