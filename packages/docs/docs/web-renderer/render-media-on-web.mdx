...
Controls whether rendering intentionally pauses between frames so the page stays responsive.

Default is `"disabled"`, which prioritizes render speed.

- `"disabled"`: Do not pause rendering.
...
This option is best effort. If one frame takes a long time to render, the page cannot update until that frame has finished.
