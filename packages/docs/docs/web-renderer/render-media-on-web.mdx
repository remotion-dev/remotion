...
### `pageResponsiveness?`<AvailableFrom v="4.0.487" />

_string | number_ <TsType type="WebRendererPageResponsiveness" source="@remotion/web-renderer" href="/docs/web-renderer/types#webrendererpageresponsiveness" />

Controls whether rendering tries to free the event loop between expensive render phases so the page stays responsive.
See [Page responsiveness](/docs/client-side-rendering/page-responsiveness) for guidance.
...
- `"low"`: Every `100ms` of rendering work, try to free the event loop.
- `"medium"`: Every `33ms` of rendering work, try to free the event loop.
- `"high"`: Every `16ms` of rendering work, try to free the event loop.

Pass a positive number to customize the interval in milliseconds.

This option is best effort. Long synchronous browser or graphics operations may still block the page until they complete.
...
### `outputWritable?`<AvailableFrom v="4.0.508" />

_WritableStream&lt;StreamTargetChunk&gt;_

Writes rendered media progressively to a `WritableStream` as data becomes available, instead of buffering the complete output in memory or the Origin Private File System.
This is useful for large renders and for streaming the output to a file, server, or native application.

The stream receives [Mediabunny `StreamTargetChunk`](https://mediabunny.dev/api/StreamTargetChunk) objects containing the data and its byte position.
[`FileSystemWritableFileStream`](https://developer.mozilla.org/en-US/docs/Web/API/FileSystemWritableFileStream) is compatible with this format.

For example, you can render directly to a file selected by the user without keeping the finished video in memory:

```tsx title="RenderButton.tsx"
import {renderMediaOnWeb} from '@remotion/web-renderer';
...
const renderToFile = async () => {
  ...
  const outputWritable = await fileHandle.createWritable();
...
export const RenderButton: React.FC = () => (
  <button onClick={renderToFile}>Render video</button>
);
```

`renderMediaOnWeb()` writes chunks at their correct byte positions and closes the stream when the render finishes. Do not call `getBlob()` when using `outputWritable`.

If both `outputWritable` and `outputTarget` are specified, `outputWritable` takes precedence.
...
See [Telemetry](/docs/telemetry) for more information.
...
### `svgFonts?`<AvailableFrom v="4.0.524" />

_array | function_

Fonts to embed inside an `<svg>` before it is rasterized, so that its `<text>` can use them.

An `<svg>` is drawn by serializing it and loading it through an `<img>`. An SVG loaded as an image is an isolated document — it cannot reach the fonts of the page around it, so its text falls back to a system face. It can only read font data that it carries itself, which is what this option supplies.

Pass an array to use the same fonts for every SVG, or a function that receives the `<svg>` element and returns the fonts for it. The function may be `async`, and returning `null` or an empty array embeds nothing. It is not called for an SVG that holds no `<text>`, `<tspan>` or `<textPath>`.

Each entry takes the font's bytes, not a URL — a remote file cannot be fetched from inside an SVG that is drawn as an image.

```tsx twoslash title="Embedding a font for SVG text"
const Component: React.FC = () => null;
const composition = {
  component: Component,
  durationInFrames: 100,
...
const data = await (await fetch('/my-font.woff2')).arrayBuffer();

const media = await renderMediaOnWeb({
  composition,
  svgFonts: [{family: 'MyFont', data}],
});
```

`family` and `data` are required. `weight`, `style` and `unicodeRange` are optional and map to the matching `@font-face` descriptors. `format` is optional too — it is read from the font's own file signature unless you pass it.

The same `ArrayBuffer` is only encoded once, however many frames it is used on, so hold on to it rather than re-reading the font per frame.

A family that ships many `unicode-range` subsets, such as a CJK font, is best handled with the function form: read the characters the `<svg>` actually paints and pass only the subsets that cover them.

### `getBlob()`

A function that returns a `Promise<Blob>` containing the rendered video. Call this function to retrieve the final video file.

If `outputWritable` was specified, the rendered media has already been written to the stream and `getBlob()` rejects.
