...
:::note
Some browsers (for example Firefox) do not expose a **native** WebCodecs AAC encoder. Before querying codecs, this function ensures [`@mediabunny/aac-encoder`](https://mediabunny.dev/guide/extensions/aac-encoder) is registered when native AAC encoding is unavailable—it is bundled with `@remotion/web-renderer`—so `"aac"` can still be returned for MP4 alongside other supported codecs.
:::
