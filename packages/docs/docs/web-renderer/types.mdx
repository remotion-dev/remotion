...
## `WebRendererPageResponsiveness`<AvailableFrom v="4.0.487" />
...
Controls intentional pauses while using [`renderMediaOnWeb()`](/docs/web-renderer/render-media-on-web#pageresponsiveness).
...
Used when encoding a still to a [`Blob`](/docs/web-renderer/render-still-on-web#bloboptions) or [object URL](/docs/web-renderer/render-still-on-web#urloptions) via [`renderStillOnWeb()`](/docs/web-renderer/render-still-on-web).
...
```tsx twoslash
import type {RenderStillOnWebEncodeOptions} from '@remotion/web-renderer';
//                ^?
```
...
```tsx twoslash
import type {RenderStillOnWebResult} from '@remotion/web-renderer';
//                ^?
```
