...
- `renderedDoneIn`: The time in milliseconds until all frames have been rendered, or `null` while rendering is still in progress
- `encodedDoneIn`: The time in milliseconds until all frames have been encoded, or `null` while encoding is still in progress
- `renderEstimatedTime`: Estimated time remaining in milliseconds until rendering is done
- `progress`: Overall progress as a number between `0` and `1`
