...
The tradeoff is render speed. Freeing the event loop gives the browser time to work, so a render may finish later than if page responsiveness is disabled.
...
Remotion only frees the event loop between safe points in the render pipeline, so it cannot interrupt a single long synchronous browser or graphics operation.
