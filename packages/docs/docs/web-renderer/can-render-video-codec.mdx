...
:::tip
For most use cases, prefer [`canRender()`](/docs/web-renderer/can-render) which mirrors `renderMediaOnWeb()` options and checks both video and audio codec support.
:::

A lower-level utility that checks if the browser supports encoding video with a specific codec.
Use this for granular codec checks when you need more control than `canRender()` provides.
...
- [`canRender()`](/docs/web-renderer/can-render)
