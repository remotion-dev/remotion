...
If unmuting a video, we recommend setting both [`.setVolume(1)`](#setvolume) and [`.setMuted(false)`](#setmuted).
...
The default implementation makes the volume slider vertical (`isNarrow`) and is keyboard navigateable using the Tab key (`onBlur`, `inputRef`).
...
export const App: React.FC = () => {
  const renderVolumeSlider: RenderVolumeSlider = useCallback(
    ({isNarrow, volume, className, onBlur, inputRef, setVolume}) => {
