...
### `volumePersistenceKey`<AvailableFrom v="4.0.xxx" />

_optional_ A string that allows you to customize the `localStorage` key used for saving and retrieving the user's volume preference.

By default, Remotion uses the key `"remotion.volumePreference"`.

This prop is useful in environments where you need to avoid key collisions or have specific naming policies for `localStorage` keys, such as in multi-tenant applications or embedded SaaS widgets.
