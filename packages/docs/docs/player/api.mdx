...
This option controls how many shared audio tags are rendered. In Remotion 4.0, the default is `5`. In Remotion 5.0, the default is `0`. If you mount more audio tags than shared audio tags are available, an error will be thrown.

Pass a value greater than `0` to enable this behavior. With `0`, each [`<Html5Audio />`](/docs/html5-audio) mounts its own native `<audio>` element when it appears.
...
### `_experimentalKeepAudioContextAlive?`<AvailableFrom v="4.0.508" />

<ExperimentalBadge>
<p>We are currently testing this prop. We might either remove the functionality or rename the prop to a stable name.</p>
</ExperimentalBadge>

Keeps the [`AudioContext`](https://developer.mozilla.org/en-US/docs/Web/API/AudioContext) running while the Player is paused, instead of suspending it. Pausing lowers the gain to zero rather than stopping the context, so the next `play()` does not have to wait for the context to come back up.

This is meant for editor-style applications (video editors, DAW-like tools) that play and pause many times per session, where the delay of resuming the context on every play is noticeable. For a Player that is played a few times, leave this off.

The context is started when the Player mounts if the browser's autoplay policy allows it, otherwise on the first click or key press. It is suspended again when the Player unmounts.

The value cannot be changed after the Player mounts. Doing so throws an error.
...
This component previews a composition in a web app. To export it, pass the composition to a [rendering API](/docs/renderer).
