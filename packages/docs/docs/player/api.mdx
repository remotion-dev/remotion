...
To access player state (playing, frame, volume, etc.) and control methods (play, pause, seekTo, etc.), use a [`PlayerRef`](#playerref).
...
const CustomPlayIndicator: React.FC<{
...
export const App: React.FC = () => {
  ...
  const renderCustomControls: RenderCustomControls = useCallback(() => {
    return <CustomPlayIndicator playerRef={playerRef} />;
  }, []);
...
See [Custom controls](/docs/player/custom-controls#controls-slot) for more examples.
