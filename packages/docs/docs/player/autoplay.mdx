...
## Controlling the behavior of autoplay failures<AvailableFrom v="4.0.187" />

If the browser blocks a video from being autoplayed due to it having sound, the video will be muted and will be retried once.  
If you want to handle the error yourself, you can pass a callback to the [`onAutoPlayError`](/docs/offthreadvideo#onautoplayerror) prop of `<OffthreadVideo>` and `<Video>`.
