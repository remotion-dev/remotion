...
## Loading a component from a URL (experimental)

<ExperimentalBadge>
<p>This API is experimental and may change.</p>
</ExperimentalBadge>

You can load a component module from a URL and pass it to `lazyComponent`.

```tsx twoslash
// @allowUmdGlobalAccess
import { Player, experimental_loadComponentFromUrl } from "@remotion/player";
...
const remoteModuleUrl = `data:text/javascript;charset=utf-8,${encodeURIComponent(remoteModuleCode)}`;

export const App: React.FC = () => {
  const lazyComponent = useMemo(() => {
    return experimental_loadComponentFromUrl({
      url: remoteModuleUrl,
    });
  }, []);
...
In production, use an `https://` URL that serves an ESM module and make sure CORS is configured.
