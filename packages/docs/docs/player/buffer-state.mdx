...
In Remotion 4.0, media tags such as `<Video>` and `<Audio>` will need to opt-in to use the buffer state.

In Remotion 5.0, media tags will automatically use the buffer state, but they can opt out of it.
