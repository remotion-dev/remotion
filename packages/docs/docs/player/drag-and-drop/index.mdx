...
By passing `from` and `durationInFrames` to `Interactive.Div`, the item is only displayed from `from` until `from + durationInFrames`.

`Interactive.Div` renders the draggable `<div>` directly, without an additional layout wrapper.
