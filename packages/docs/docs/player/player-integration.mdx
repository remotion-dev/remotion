...
:::note
Remotion and your React app use different bundler configurations. Therefore, if you want to override your [bundler configuration](/docs/bundlers), you need to override both the Remotion one and the React app one.
:::
...
which have the Player and cloud rendering already set up.
...
:::tip
When using Lambda, pass the resulting directory to [`deploySiteFromBundle()`](/docs/lambda/deploysitefrombundle). Alternatively, use the Lambda CLI, which bundles and deploys the project in one command.
:::
