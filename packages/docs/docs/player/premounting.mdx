...
## Premounting media<AvailableFrom v="4.0.495" />

[`<Video>`](/docs/media/video) and [`<Audio>`](/docs/media/audio) from `@remotion/media` accept `premountFor` directly. They remain layoutless, carry `display: none` while premounted and register the premounted range in the Studio timeline.

```tsx twoslash title="PremountedVideo.tsx"
import {Video} from '@remotion/media';
import {staticFile} from 'remotion';

export const MyVideo = () => {
  return <Video src={staticFile('video.mp4')} from={60} premountFor={30} />;
};
```
