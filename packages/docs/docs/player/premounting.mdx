...
A premounted [`<Sequence>`](/docs/sequence) with the default layout carries the styles `opacity: 0` and `pointer-events: none`.
For `layout="none"`, Remotion uses an internal wrapper that does not affect layout during normal playback and becomes an invisible out-of-flow host while premounting or postmounting.
...
:::note
The built-in [`<Sequence>`](/docs/sequence) component supports premounting with `layout="none"`.
This custom implementation does not support it because it applies hidden styles to the sequence container directly.
