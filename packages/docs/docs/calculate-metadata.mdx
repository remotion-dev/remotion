...
<div>
<Step>5</Step> want to add a per-composition default <a href="/docs/sample-rate">audio sample rate</a>
</div>
<div>
<Step>6</Step> want to run code once, before the actual render starts.
</div>
...
- `defaultSampleRate`: The default [audio sample rate](/docs/sample-rate) in Hz. Optional. <AvailableFrom v="4.0.448" />
