...
<div>
<Step>3</Step> want to add per-composition default render settings (codec, image format, pixel format)
</div>
<div>
...
const calculateMetadata: CalculateMetadataFunction<MyComponentProps> = ({props, defaultProps, abortSignal}) => {
  return {
    ...
    defaultCodec: 'h264',
    // or add per-composition default image format for video rendering
...
- `defaultVideoImageFormat` _optional_: The default image format to use when rendering frames for video. Either `'png'`, `'jpeg'`, or `'none'`. <AvailableFrom v="4.0.315" />
- `defaultPixelFormat` _optional_: The default pixel format to use for the composition. One of `'yuv420p'`, `'yuva420p'`, `'yuv422p'`, `'yuv444p'`, `'yuv420p10le'`, `'yuv422p10le'`, `'yuv444p10le'`, `'yuva444p10le'`. <AvailableFrom v="4.0.315" />
...
The same priority rules apply to `defaultVideoImageFormat` and `defaultPixelFormat` - they will be used as fallbacks if not explicitly specified in the render options.
