...
First, create an S3 bucket in your preferred region. If one already exists, it will be used instead:
...
Next, build your Remotion project and upload the resulting bundle to the bucket. The entry point is the file where [`registerRoot()`](/docs/register-root) is called.
...
When you update your Remotion code, create and deploy a new bundle. Pass the same [`siteName`](/docs/lambda/deploysitefrombundle#sitename) to overwrite the previous deployment. If you don't pass [`siteName`](/docs/lambda/deploysitefrombundle#sitename), a unique URL will be generated on every deployment.
...
By default, it is `1000` concurrent invocations per region. However, new accounts might have a limit [as low as `10`](/docs/lambda/troubleshooting/rate-limit#new-accounts-using-aws-lambda). Each Remotion render may use as much as 200 functions per render concurrently, so if your assigned limit is very low, [request an increase](/docs/lambda/troubleshooting/rate-limit#request-an-increase). You can use [single-function rendering](/docs/lambda/troubleshooting/rate-limit#render-with-one-function) while waiting for the increase.
