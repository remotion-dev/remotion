...
The Lambda runtime ships with the essential codecs to keep it lean, so it does not include the AV1 encoder. AV1 decode support remains available, and AV1 encoding is still available for non-Lambda renders.
