...
The Lambda uses a NodeJS version from the `18.x` release line.  
...
| --------------------- | -------------- |
| From 5.0.0 (upcoming) | 123.0.6312.86  |
...
## Runtime changes in Remotion 5.0
...
If you use the CLI, pass `enable-v5-runtime` to the CLI.
