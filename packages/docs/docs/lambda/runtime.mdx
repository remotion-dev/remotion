...
To stay within AWS package size limits, the Lambda runtime does not include the AV1 encoder. AV1 decode support remains available, and AV1 encoding is still available for non-Lambda renders.
