...
### `customLayerArns?`<AvailableFrom v="4.0.510" />

An ordered, non-empty array of complete Lambda Layer version ARNs. The specified Layers replace all Remotion-hosted runtime Layers.

Each ARN must include a numeric version and use the function's AWS region and AWS partition. If omitted, Remotion-hosted Layers continue to be selected using `runtimePreference`. Non-default `runtimePreference` values cannot be combined with custom Layers.

This option is required in [AWS China regions](/docs/lambda/aws-china-regions), where Remotion-hosted Layers are unavailable.

See [Custom Layers](/docs/lambda/custom-layers) for usage, limits, compatibility, and IAM requirements.
...
<CompatibilityTable chrome={false} firefox={false} safari={false} nodejs bun serverlessFunctions="Deployment files required" clientSideRendering="" serverSideRendering="" player="" studio="" hideBrowsers />

The Remotion Lambda deployment files must be included in the serverless deployment.
