...
### `customLayerArns?`<AvailableFrom v="4.0.508" />
...
Each ARN must include a numeric version and use the function's AWS region. If omitted, Remotion-hosted Layers continue to be selected using `runtimePreference`. Non-default `runtimePreference` values cannot be combined with custom Layers.
