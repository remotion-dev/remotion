---
image: /generated/articles-docs-lambda-upgrading.png
id: upgrading
title: Upgrading Lambda
slug: /lambda/upgrading
---

import {Prerelease} from '../../components/PrereleaseVersion';

## 1. Upgrade all Remotion packages

Upgrade all packages to the newest version (`@remotion/lambda`, but also `remotion`, `@remotion/cli` etc.)

```sh
npx remotion upgrade
```

<Prerelease packageName="@remotion/lambda" />

## 2. Deploy a new Lambda function

```sh
npx remotion lambda functions deploy
```

This deploys a new function with the latest version. You can keep old functions deployed if they are still in use in production.

## 3. Update the site

:::warning
This step is required. The function, the site and the `@remotion/lambda` package calling the render should all be on the same version.
If the site is on a different version than the function, you may get a version mismatch warning – or if there was a protocol change, **renders will fail**.
:::

```sh
npx remotion lambda sites create src/index.ts --site-name=my-name
```

Pass `--site-name` with the name of an existing site to update it. The URL will stay the same but older functions may not be able to render the updated site.

If you don't pass `--site-name`, a new site URL will be generated. You'll need to update the [`serveUrl`](/docs/lambda/rendermediaonlambda#serveurl) parameter in your [`renderMediaOnLambda()`](/docs/lambda/rendermediaonlambda) calls.

## 4. (Optional) Remove old functions

:::info
Only do this once the old function is no longer being used in production.
:::

```sh
npx remotion lambda functions rmall -y
```

## Zero-downtime upgrades

If you already shipped Remotion Lambda to production, you can upgrade without incurring any downtime:

1. Deploy the new function and create a new site – old functions and sites remain unaffected.
2. Update the `@remotion/lambda` package version, function name and `serveUrl` in your application.
3. Ship your application update. From this point, all new renders use the new function + site.
4. Once no in-flight renders remain on the old version, remove the old function and site.

### Useful commands

- List deployed functions and their versions: `npx remotion lambda functions ls`
- Find functions matching your installed version: use the [`compatibleOnly`](/docs/lambda/getfunctions#compatibleonly) parameter.
- List deployed sites: `npx remotion lambda sites ls`

See also: [Separating production and testing environments](/docs/lambda/separate-environments)
