...
Uploads an existing local [Remotion Bundle](/docs/terminology/bundle) to an S3 bucket without bundling the source code again.
...
:::caution
Deploy only trusted artifacts. Files referenced by symbolic links inside the bundle are included in the upload.
:::
...
Default: the `build` directory in the Remotion project root, matching the output of `npx remotion bundle`.

Pass this property when the bundle is stored elsewhere:
