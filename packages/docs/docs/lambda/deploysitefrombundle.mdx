...
Use this API together with [`bundle()`](/docs/bundle) to separate building from deployment. This allows an artifact to be built once and deployed in a later Docker or CI stage. Before calling it, retrieve a bucket using [`getOrCreateBucket()`](/docs/lambda/getorcreatebucket).
