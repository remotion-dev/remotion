...
From <AvailableFrom v="4.0.499" inline />, [`deploySite({bundleDir})`](/docs/lambda/deploysite) provides the same behavior and is the preferred API for new code. `deploySiteFromBundle()` remains available as an alias.
