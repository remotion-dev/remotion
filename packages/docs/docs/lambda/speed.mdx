...
[`getRenderProgress()`](/docs/lambda/getrenderprogress) can read progress directly from S3 instead of invoking a Lambda function for every poll. See [Faster progress polling](/docs/lambda/faster-progress-polling) for the benefits and requirements.
