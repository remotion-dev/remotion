...
- Renderer functions render a short portion of a video and then shut down. From <AvailableFrom v="4.0.517" inline />, with `concurrency: 1` and no `rendererFunctionName`, the main function renders the video directly and no renderer function is invoked.
