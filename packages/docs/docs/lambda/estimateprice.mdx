...
- The main function normally spawns worker functions, waits for chunks to be rendered, and stitches them together for the full video. This is the longest-running Lambda function.
- Renderer functions render a short portion of a video and then shut down. From <AvailableFrom v="4.0.517" inline />, you can set `concurrency` to `1` to render the video on the main function.
