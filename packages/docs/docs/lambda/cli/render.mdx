...
Press Ctrl+C to cancel the render. Without this flag, Ctrl+C stops the CLI from waiting while the Lambda render continues.
...
From <AvailableFrom v="4.0.517" inline />, you can set `--concurrency=1` to render the video on the main function.

Maximum value: 200  
Minimum value: 1 if rendering on main function, otherwise value must result in `framesPerLambda >= 4`
