...
[`h264` or `h265` or `vp8` or `vp9` or `gif` or `mp3` or `aac` or `wav` or `prores`](/docs/config#setcodec). If you don't supply `--codec`, it will use `h264`.
...
### `--gop`<AvailableFrom v="4.0.466" />

<Options id="gop" />

On Lambda, this sets the maximum GOP size per chunk. Additional keyframes will appear at chunk boundaries, every `--frames-per-lambda` frames.
...
The endpoint for an [`s3OutputProvider`](/docs/lambda/custom-destination#saving-to-another-cloud). Use it to save the rendered media to an S3-compatible provider such as Cloudflare R2.

When using this option, also pass `--force-bucket-name` and `--out-name`. If your provider requires credentials, pass `--s3-output-provider-access-key-id` and `--s3-output-provider-secret-access-key`.

```bash
npx remotion lambda render testbed my-comp \
  --force-bucket-name=my-r2-bucket \
  --out-name=renders/video.mp4 \
  --privacy=no-acl \
  --s3-output-provider-endpoint=https://<account-id>.r2.cloudflarestorage.com \
  --s3-output-provider-access-key-id=<access-key-id> \
  --s3-output-provider-secret-access-key=<secret-access-key> \
  --s3-output-provider-region=auto
```

### `--s3-output-provider-access-key-id`

The access key ID to use with `--s3-output-provider-endpoint`.

### `--s3-output-provider-secret-access-key`
