...
AV1 is not available on Remotion Lambda because the Lambda binary must stay within AWS package size limits. AV1 remains available for non-Lambda renders.
