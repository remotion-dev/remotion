...
Remotion Lambda ships with the essential codecs to keep the runtime lean. The same lean binary set is also used on Linux ARM64 GNU, so `av1` is not available there either. `AV1` remains available on the other shipped targets.
