...
A JSON string matching [`s3OutputProvider`](/docs/lambda/custom-destination#saving-to-another-cloud). Use it to save the rendered media to an S3-compatible provider such as Cloudflare R2.

When using this option, also pass `--force-bucket-name` and `--out-name`:
...
npx remotion lambda render testbed my-comp \
  ...
  --privacy=no-acl \
  --s3-output-provider='{"endpoint":"https://<account-id>.r2.cloudflarestorage.com","accessKeyId":"<access-key-id>","secretAccessKey":"<secret-access-key>","region":"auto"}'
```
