...
From <AvailableFrom v="4.0.517" inline />, setting `--concurrency=1` without specifying `--renderer-function-name` makes the main function render the video directly. No additional renderer function is invoked.
