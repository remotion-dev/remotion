...
Bundles and uploads a Remotion project to an S3 bucket.

You may pass an [entry point](/docs/terminology/entry-point), or omit the argument to [determine it automatically](/docs/terminology/entry-point#which-entry-point-is-being-used).

From <AvailableFrom v="4.0.499" inline />, you can alternatively pass an existing [Remotion Bundle](/docs/terminology/bundle) to skip bundling:
...
Passing a bundle directory is compatible with Remotion v5, where entry points are no longer accepted.
...
<details>
  <summary>Example output when passing an entry point</summary>
  <pre>
...
Only applies when passing an entry point.
