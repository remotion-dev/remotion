...
## `--custom-layer-arns`<AvailableFrom v="4.0.508" />

A comma-separated list of complete Lambda Layer version ARNs. The specified Layers replace all Remotion-hosted runtime Layers.

```sh title="Deploy with custom Layers"
...
See [Custom Layers](/docs/lambda/custom-layers) for limits, compatibility, and IAM requirements.
