...
The AWS partition for which to generate the policy. Accepts `"aws"` (default) or `"aws-cn"`. See [AWS China regions](/docs/lambda/aws-china-regions).

## Compatibility

<CompatibilityTable chrome={false} firefox={false} safari={false} nodejs bun serverlessFunctions clientSideRendering="" serverSideRendering="" player="" studio="" hideBrowsers />

For serverless functions, import this API from `@remotion/lambda/policies`.
