...
For new code, prefer building with [`bundle()`](/docs/bundle) and uploading its output with [`deploySiteFromBundle()`](/docs/lambda/deploysitefrombundle). Separating these steps avoids rebuilding an unchanged bundle during deployment.
