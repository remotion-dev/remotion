...
Takes a Remotion project, bundles it and uploads it to an S3 bucket. Once uploaded, a Lambda function can render any composition in the project by specifying the URL.

From <AvailableFrom v="4.0.499" inline />, you can alternatively pass an existing `bundleDir` to skip bundling. This call shape is compatible with Remotion v5.
...
To build and deploy in separate steps, pass the output of [`bundle()`](/docs/bundle) as `bundleDir` instead:
...
Pass exactly one of `entryPoint` and `bundleDir`.

### `entryPoint`?

An absolute path pointing to the [entry point](/docs/terminology/entry-point) of your Remotion project. Passing an entry point bundles and deploys the project in one operation.
...
The following build options are only available when passing `entryPoint`. Pass them to [`bundle()`](/docs/bundle) when building separately.
