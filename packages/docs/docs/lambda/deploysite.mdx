...
`deploySite()` remains available and continues to bundle a Remotion project and upload it to an S3 bucket. Once uploaded, a Lambda function can render any composition in the Remotion project by specifying the URL.
...
<CompatibilityTable chrome={false} firefox={false} safari={false} nodejs bun serverlessFunctions={false} clientSideRendering="" serverSideRendering="" player="" studio="" hideBrowsers />
