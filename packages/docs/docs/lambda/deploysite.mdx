...
Uploads a local [Remotion Bundle](/docs/terminology/bundle) to an S3 bucket. Once uploaded, a Lambda function can render any composition in the project by specifying the URL.

From <AvailableFrom v="4.0.499" inline />, pass `bundleDir` to keep building and deployment as separate steps. In Remotion v4, you may instead pass `entryPoint` to bundle and deploy in one call. The `entryPoint` mode is removed in Remotion v5.
...
Deploy the resulting directory:
...
Pass exactly one of `bundleDir` and `entryPoint`.

### `bundleDir`?<AvailableFrom v="4.0.499" />

The local directory containing the bundle. It must contain `index.html` and `bundle.js` files at its root.

The bundle must be relocatable. Bundles created using [`npx remotion bundle`](/docs/cli/bundle) or [`bundle()`](/docs/bundle) are relocatable by default.

The default output directory of `npx remotion bundle` is `./build`. If you use `bundle()`, pass its return value.
...
An absolute path pointing to the [entry point](/docs/terminology/entry-point) of your Remotion project. Passing an entry point bundles and deploys the project in one operation and is only supported in Remotion v4.

**Required action for Remotion v5:** Call [`bundle()`](/docs/bundle) separately and pass its output as `bundleDir`.
...
Only available when passing `entryPoint`. Callback from the bundler when bundling has progressed. Passes a number between 0 and 100.
...
The following build options are only available when passing `entryPoint` in Remotion v4. Pass them to [`bundle()`](/docs/bundle) when building separately.

#### `webpackOverride?`

Passes a custom Webpack override. It is ignored if `rspack` is `true`. See [`bundle()` → `webpackOverride`](/docs/bundle#webpackoverride).

#### `bundlerOverride?`<AvailableFrom v="4.0.498" />

Passes an override shared by Webpack and Rspack. See [`bundle()` → `bundlerOverride`](/docs/bundle#bundleroverride).

#### `rspackOverride?`<AvailableFrom v="4.0.498" />

Passes a custom Rspack override. See [`bundle()` → `rspackOverride`](/docs/bundle#rspackoverride).
...
- [`deploySiteFromBundle()`](/docs/lambda/deploysitefrombundle), an alias for deploying a bundle directory
