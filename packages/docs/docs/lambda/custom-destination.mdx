...
- When calling [`downloadMedia()`](/docs/lambda/downloadmedia#customcredentials) or [`getRenderProgress()`](/docs/lambda/getrenderprogress#s3outputprovider), you must provide the same credentials again, as the `customCredentials` option of `downloadMedia()` and the `s3OutputProvider` option of `getRenderProgress()`.
