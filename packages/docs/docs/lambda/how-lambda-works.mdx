...
<Step>4</Step> Based on the determined duration of the video and the <a href="/docs/lambda/concurrency">
  concurrency
</a>, the video is split into chunks. Normally, a <strong>renderer function</strong> is invoked for each chunk. From <AvailableFrom v="4.0.517" inline />, you can set <code>concurrency</code> to <code>1</code> to render the video on the main function.<br />
