...
Remotion Lambda is a distributed video rendering system. The video is split into chunks that can be rendered concurrently. You can define the number of chunks or use the Remotion defaults.
...
From <AvailableFrom v="4.0.517" inline />, setting `concurrency` to `1` without specifying a `rendererFunctionName` makes the main function render the video directly. No additional renderer function is invoked.
