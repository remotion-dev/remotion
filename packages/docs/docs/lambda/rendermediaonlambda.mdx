...
Pass an array of ordered, non-overlapping ranges to concatenate multiple sections into one video. Video and audio from gaps are omitted. Ranges are inclusive, and an open-ended range may only appear last.<AvailableFrom v="4.0.502" inline />

```ts twoslash title="render-on-lambda.ts"
import {renderMediaOnLambda} from '@remotion/lambda/client';
declare const options: Parameters<typeof renderMediaOnLambda>[0];
// ---cut---
