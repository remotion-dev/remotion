...
Enable the render to be cancelled using [`cancelRenderOnLambda()`](/docs/lambda/cancelrenderonlambda). Default: `false`.

When enabled, every renderer checks S3 once per second for a cancellation signal. This incurs additional S3 `HEAD` request charges for the duration of the render.

See [Cancelling Lambda renders](/docs/lambda/cancellation) for the full workflow.
...
From <AvailableFrom v="4.0.517" inline />, setting `concurrency` to `1` without specifying a [`rendererFunctionName`](#rendererfunctionname) makes the main function render the video directly. No additional renderer function is invoked.
...
A URL pointing to a Remotion project. Create one by building with [`bundle()`](/docs/bundle) and deploying with [`deploySiteFromBundle()`](/docs/lambda/deploysitefrombundle).
...
Starting in Remotion 5.0, the default for H.264 Lambda renders is `veryfast`. In Remotion 4.0, the default is `medium`.
...
Returns a promise resolving to an object. `renderId` and `bucketName` are useful for passing to [`getRenderProgress()`](/docs/lambda/getrenderprogress).
...
A link to the CloudWatch logs of the renderer functions, if you have not disabled CloudWatch. With `concurrency: 1` and no `rendererFunctionName`, no separate renderer function is invoked; use `cloudWatchMainLogs` instead.

### `cloudWatchMainLogs`<AvailableFrom v="4.0.174" />

A link to the CloudWatch logs of the main function. This is also where rendering logs are written when `concurrency` is `1` and no `rendererFunctionName` is specified.

### `lambdaInsightsLogs`<AvailableFrom v="4.0.61"/>
...
### `progressJsonInConsole`<AvailableFrom v="4.0.182" />

A link to the `progress.json` file in the AWS console.
