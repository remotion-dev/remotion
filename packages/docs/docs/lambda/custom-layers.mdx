...
Pass every runtime Layer to [`deployFunction()`](/docs/lambda/deployfunction#customlayerarns) or [`--custom-layer-arns`](/docs/lambda/cli/functions/deploy#--custom-layer-arns). The list replaces all Remotion-hosted Layers.

Credentials used to deploy custom Layers need `lambda:GetLayerVersion` permission for the specified Layer ARNs. Updating an existing function additionally needs `lambda:GetFunctionConfiguration` and `lambda:UpdateFunctionConfiguration`.

Custom Layers are required in [AWS China regions](/docs/lambda/aws-china-regions).
