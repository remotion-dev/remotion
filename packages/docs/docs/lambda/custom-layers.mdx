...
## Specify custom Layers while deploying

Pass every runtime Layer to [`deployFunction()`](/docs/lambda/deployfunction#customlayerarns) or `--custom-layer-arns`. The list completely replaces Remotion's hosted Chromium, font, emoji, and CJK Layers.
...
await deployFunction({
  ...
  customLayerArns: ['arn:aws:lambda:us-east-1:123456789012:layer:custom-chromium:1', 'arn:aws:lambda:us-east-1:123456789012:layer:custom-fonts:3'],
});
```

```sh title="Deploy using the CLI"
npx remotion lambda functions deploy --custom-layer-arns="arn:aws:lambda:us-east-1:123456789012:layer:custom-chromium:1,arn:aws:lambda:us-east-1:123456789012:layer:custom-fonts:3"
```

Each ARN must include a numeric Layer version and use the function's AWS region. AWS Lambda allows at most five Layers and limits the combined unzipped size of the function and all Layers to 250MB. The Lambda Insights Layer occupies one of the five slots.

You are responsible for keeping custom Layers compatible with Remotion. When custom Layers are omitted, Remotion continues to use its hosted Layers.

:::note
Credentials used to deploy custom Layers need `lambda:GetLayerVersion` permission for the specified Layer ARNs. Updating Layers on an existing function additionally needs `lambda:GetFunctionConfiguration` and `lambda:UpdateFunctionConfiguration`. These permissions must be configured separately.
:::
