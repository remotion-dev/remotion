...
Normally, a render uses a launch invocation, renderer invocations, and short progress invocations. The number of renderer invocations depends on the [concurrency](/docs/lambda/concurrency) of the render.

From <AvailableFrom v="4.0.517" inline />, setting `concurrency` to `1` without specifying a `rendererFunctionName` makes the launch invocation render the video directly, so no separate renderer invocation is used. A progress request may briefly use another invocation.

If you exceed the concurrency limit, you will get a `TooManyRequestsException` error from AWS Lambda.
