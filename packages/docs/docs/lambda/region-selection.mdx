...
:::note
The AWS China regions `cn-north-1` and `cn-northwest-1` are supported from <AvailableFrom v="4.0.510" inline /> and require additional setup. See [AWS China regions](/docs/lambda/aws-china-regions).
:::
