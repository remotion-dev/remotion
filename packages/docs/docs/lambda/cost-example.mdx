...
export const calculateMetadataFn: CalculateMetadataFunction<Record<string, unknown>> = async () => {
  ...
  return {
    durationInFrames: Math.round(durationInSeconds * compositionFps), // 14315
    fps: compositionFps,
    width: dimensions!.width,
    height: dimensions!.height,
  };
...
- [Remotion Licensing](/docs/license/pricing): Teams of 4+ people also need to acquire a Remotion Company License in addition to the AWS cost.
