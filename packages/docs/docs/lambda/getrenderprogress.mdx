...
By default, calling this function results in a Lambda invocation. See [Faster progress polling](/docs/lambda/faster-progress-polling) to read the progress directly from S3 instead.
...
This is cheaper and faster, but requires additional permissions. Default: `false`. See [Faster progress polling](/docs/lambda/faster-progress-polling) for the requirements.
