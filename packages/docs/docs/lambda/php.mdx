...
# Triggering renders from PHP<AvailableFrom v="3.3.96" />
...
Enable cancellation before starting the render, then call `cancelRenderOnLambda()`:

```php title="cancel-render.php"
$params = new RenderParams(enableCancellation: true);
$params->setComposition('react-svg');

$renderResponse = $client->renderMediaOnLambda($params);

$client->cancelRenderOnLambda(
