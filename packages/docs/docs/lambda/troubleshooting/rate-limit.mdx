...
### Render with one function<AvailableFrom v="4.0.517" />

While waiting for AWS to approve a higher limit, set [`concurrency`](/docs/lambda/rendermediaonlambda#concurrency) to `1`:
...
If no [`rendererFunctionName`](/docs/lambda/rendermediaonlambda#rendererfunctionname) is specified, the launch function renders the video directly and no renderer function is invoked. A progress request may briefly use one additional invocation.
