...
From <AvailableFrom v="4.0.517" inline />, you can set `concurrency` to `1` to render the video on the main function. There are no separate renderer logs, and the CLI prints only the main function link.
