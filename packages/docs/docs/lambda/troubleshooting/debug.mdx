...
From <AvailableFrom v="4.0.517" inline />, with `concurrency: 1` and no `rendererFunctionName`, the main function renders directly and there are no separate renderer logs. The CLI prints only the main function link.
