...
- When using [`renderMediaOnLambda()`](/docs/lambda/rendermediaonlambda), add `logLevel: "verbose"`. Use the returned `cloudWatchLogs` URL for renderer functions and `cloudWatchMainLogs` for the main function.

From <AvailableFrom v="4.0.517" inline />, with `concurrency: 1` and no `rendererFunctionName`, the main function renders directly and there are no separate renderer logs. The CLI prints only the main function link; API users should open `cloudWatchMainLogs`.
...
The renderer function link uses this filter:
