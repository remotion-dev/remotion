...
# Triggering renders from Python<AvailableFrom v="4.0.15" />
...
Enable cancellation before starting the render, then call `cancel_render_on_lambda()`:

```python title="cancel_render.py"
render_params = RenderMediaParams(
    composition="react-svg",
...
if render_response:
    client.cancel_render_on_lambda(
