...
~~Sending large input props (>200KB) is not supported with Python at the moment.~~ _available from v4.0.315_

Starting from v4.0.315, the Python SDK automatically handles large input props by uploading them to S3 when they exceed AWS Lambda payload limits. Props larger than 200KB for video renders and 5MB for still renders are automatically compressed using the same logic as the JavaScript SDK.
...
# For large input props, optionally specify a bucket name
# If not specified, a bucket will be created automatically
# client = RemotionClient(region=REMOTION_APP_REGION,
#                         serve_url=REMOTION_APP_SERVE_URL,
#                         function_name=REMOTION_APP_FUNCTION_NAME,
#                         bucket_name="my-remotion-bucket")
...
When large props are detected, they are automatically uploaded to S3 and referenced by the Lambda function. This process is transparent to the user.
