...
# Triggering renders from Ruby<AvailableFrom v="4.0.232" />
...
- The implemented methods are [`renderMediaOnLambda()`](/docs/lambda/rendermediaonlambda), [`renderStillOnLambda()`](/docs/lambda/renderstillonlambda), [`getRenderProgressOnLambda()`](/docs/lambda/getrenderprogress), and [`cancelRenderOnLambda()`](/docs/lambda/cancelrenderonlambda).
...
Enable cancellation in the render payload, then call `cancel_render_on_lambda()`:

```ruby title="Cancelling a render from Ruby"
payload = get_render_media_on_lambda_payload(
  composition: "still-helloworld",
  codec: "h264",
...
res = client.render_media_on_lambda(function_name, payload)
client.cancel_render_on_lambda(res["bucketName"], res["renderId"])
```
