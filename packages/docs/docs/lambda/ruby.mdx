...
- The implemented methods are [`renderMediaOnLambda()`](/docs/lambda/render-media-on-lambda), [`renderStillOnLambda()`](/docs/lambda/render-still-on-lambda) and [`getRenderProgressOnLambda()`](/docs/lambda/get-render-progress).
