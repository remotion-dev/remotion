...
Use the default `<Sequence>` layout if the component should show a canvas outline in the Studio. A `<Sequence layout="none">` can still expose timeline controls, but it does not register an outline automatically.
...
`withInteractivitySchema()` is also exported from `remotion` as an alias for `Interactive.withSchema()`.
