...
const audioData = useAudioData(staticFile('voice-short.mp3')) as AudioData;
...
const BaseExample: React.FC = () => {
  const frame = useCurrentFrame();
  const audioDataVoice = useAudioData(staticFile('voice-short.mp3'));
  ...
  return (
    <div style={{flex: 1}}>
      <Audio src={staticFile('voice-short.mp3')} />
...
const frame = useCurrentFrame();
const audioDataVoice = useAudioData(staticFile('voice-short.mp3')) as AudioData;
const {fps} = useVideoConfig();
