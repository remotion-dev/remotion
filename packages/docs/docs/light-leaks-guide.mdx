...
Use [`lightLeak()`](/docs/light-leaks/light-leak-effect) from [`@remotion/light-leaks`](/docs/light-leaks/api) to apply the same visual to canvas-based components with an `effects` array.
...
- [`lightLeak()` effect](/docs/light-leaks/light-leak-effect)
