...
Use [`lightLeak()`](/docs/light-leaks/light-leak-effect) from [`@remotion/effects`](/docs/effects/api) to apply the same visual to canvas-based components with an `effects` array.
