...
for (const file of webcamFiles) {
  ...
  const cmd = await $`ffmpeg -hide_banner -i ${path} -af loudnorm=I=-23:LRA=7:print_format=json -f null -`.quiet();
  const output = cmd.stderr.toString('utf8');
  const lines = output.split('\n');
  const indexOfLineBeforeStart = lines.findIndex((line) => line.includes('[Parsed_loudnorm_0 @'));
