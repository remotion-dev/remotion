# Scenes

A video is made up of one or multiple scenes.

To add a new scene, click on `+` in the right sidebar.

<br />
<div style={{display: "flex", justifyContent: "center"}}>
<video
  src="/add-new-scene.mov"
  width="700"
  height="400"
  autoPlay
  muted
  controls
  loop
/>

</div>

## Types of scenes

There are multiple type of scenes you can choose from:

<br />

<div
  style={{
    display: "flex",
    flexDirection: "row",
    justifyContent: "space-around",
  }}
>
  <div
    style={{
      display: "flex",
      flexDirection: "column",
      alignItems: "center",
      gap: 6,
    }}
  >
    <img src="/titlecard-example.png" width="250" height="250" />
    <div>Title</div>
  </div>
  <div
    style={{
      display: "flex",
      flexDirection: "column",
      alignItems: "center",
      gap: 6,
    }}
  >
    <img src="/videoscene-example.png" width="250" height="250" />
    <div>Camera Scene</div>
  </div>
  <div
    style={{
      display: "flex",
      flexDirection: "column",
      alignItems: "center",
      gap: 6,
    }}
  >
    <img src="/endcard-example.png" width="250" height="250" />
    <div>Endcard</div>
  </div>
</div>

### Title Scene

A title consists of the main title and an optional subtitle.
The subtitle can be removed by checking the `null` checkbox in the right side.

### Camera Scene

A camera scene contains a webcam recording, an optional screencast and captions, if [generated](/docs/recorder/captions). Each recording captured with the recorder corresponds to one camera scene.
For each newly added camera scene, the chronologically next recording will be added to the timeline.

The webcam can be positioned in either one of the four corners of the video. The other components will be automatically positioned around the webcam.

### Endcard

In the endcard you can add call to actions, such as links to your social media profiles or other websites.
