...
A `boolean` specifying whether the studio should show the Ask AI modal on CMD+I shortcut. Default `true`, disable this if it conflicts with your studio.

### `keyboardShortcutsEnabled?`

_optional_

A `boolean` specifying whether the studio respond to the predefined keyboard shortcuts.. Default `true`.
