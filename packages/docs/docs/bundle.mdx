...
Bundles a Remotion project using Webpack, or Rspack if enabled, and prepares it for rendering using [`renderMedia()`](/docs/renderer/render-media). [See a full server-side rendering example.](/docs/ssr-node)
...
```ts twoslash
import {RspackOverrideFn} from '@remotion/bundler';
// ---cut---
...
From <AvailableFrom v="4.0.497" inline />, the default `./` makes the bundle relocatable. Earlier versions defaulted to `/`. Pass `/` to retain an origin-root URL.
...
Pass this directory to [`deploySiteFromBundle()`](/docs/lambda/deploysitefrombundle) to upload it to Remotion Lambda without bundling again.
...
- [`deploySiteFromBundle()`](/docs/lambda/deploysitefrombundle)
