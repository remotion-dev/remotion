...
Since <AvailableFrom v="4.0.516" inline />, Remotion also [infers basic visual editing controls](/docs/default-props-inference) from the values in `defaultProps`.

## Optional: Define a schema<AvailableFrom v="4.0.0"/>

You can use [Zod](https://github.com/colinhacks/zod) to [define a typesafe schema for your composition](/docs/schemas). A schema is not required for visual editing. See [When to define a schema](/docs/default-props-inference#when-to-define-a-schema) for cases where an explicit schema is useful.
