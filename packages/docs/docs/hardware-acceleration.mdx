...
- FFmpeg compiled with NVENC support (the Remotion-bundled FFmpeg includes it)

No additional configuration is needed. Set `--hardware-acceleration if-possible` and Remotion will automatically use NVENC if available.
