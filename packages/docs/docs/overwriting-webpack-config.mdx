...
Remotion ships with its own [Webpack configuration](https://github.com/remotion-dev/remotion/blob/main/packages/bundler/src/webpack-config.ts) and [Rspack configuration](https://github.com/remotion-dev/remotion/blob/main/packages/bundler/src/rspack-config.ts).

You can override them reducer-style by creating a function that takes the previous configuration and returns the next one.

Use [`Config.overrideWebpackConfig()`](/docs/config#overridewebpackconfig) for Webpack-specific changes and [`Config.overrideRspackConfig()`](/docs/config#overriderspackconfig) for Rspack-specific changes.

[`Config.overrideBundlerConfig()`](/docs/config#overridebundlerconfig) applies compatible changes to both bundlers. The shared override runs first, followed by the selected bundler's specific override.

Webpack and Rspack plugins are not interchangeable. Do not add bundler-specific plugins in `Config.overrideBundlerConfig()`.

## Webpack overrides using the command line
...
## Rspack overrides using the command line<AvailableFrom v="4.0.498" />
