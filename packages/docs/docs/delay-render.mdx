...
If a is shaky (for example, if loading an asset from a CDN does is flaky), you can pass an object with a `timeoutInMilliseconds` value as a second argument.  
...
The [`<Img>`](/docs/img#delayrenderretries), [`<Audio>`](/docs/audio#delayrenderretries), [`<Video>`](/docs/video#delayrenderretries) and [`<IFrame>`](/docs/iframe#delayrenderretries) tags support a `delayRenderRetries` prop to control the value of the `delayRender()` value that those components make.
...
The [`<Img>`](/docs/img#delayrendertimeoutinmilliseconds), [`<Audio>`](/docs/audio#delayrendertimeoutinmilliseconds), [`<Video>`](/docs/video#delayrendertimeoutinmilliseconds) and [`<IFrame>`](/docs/iframe#delayrendertimeoutinmilliseconds) tags support a `delayRenderTimeoutInMilliseconds` prop to control the value of the `delayRender()` value that those components make.
