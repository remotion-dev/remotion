...
Capturing a composition that contains an [`<HtmlInCanvas>`](/docs/remotion/html-in-canvas) component through this path requires Chrome or Chromium `152.0.7944.0` or later. The renderer capture canvas and the component form a nested HTML-in-canvas tree. On older versions, Remotion falls back to the built-in DOM composer.
