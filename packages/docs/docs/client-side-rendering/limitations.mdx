...
[`<HtmlInCanvas>`](/docs/remotion/html-in-canvas) is <a style={{color: 'green'}}>supported</a> when the component itself is supported. Capturing a composition containing `<HtmlInCanvas>` through the [HTML-in-canvas capture path](/docs/client-side-rendering/html-in-canvas) requires Chrome or Chromium `152.0.7944.0` or later.<br />
