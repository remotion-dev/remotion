...
The `text-decoration` property is <a style={{color: 'green'}}>supported</a> for `solid`, `dotted`, `dashed`, and `double` `text-decoration-style` values on `underline`, `overline`, and `line-through` lines with `text-decoration-color` and `text-decoration-thickness`.
`wavy` `text-decoration-style` values and pixel-perfect line positions are <a style={{color: 'red'}}>not supported</a>.
