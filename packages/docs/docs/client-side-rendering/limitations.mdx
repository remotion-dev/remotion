...
[`<HtmlInCanvas>`](/docs/remotion/html-in-canvas) is <a style={{color: 'green'}}>supported</a> when the component itself is supported.  
...
By default, [`renderMediaOnWeb()`](/docs/web-renderer/render-media-on-web) intentionally pauses between expensive render phases to keep the page responsive.
Set [`pageResponsiveness`](/docs/web-renderer/render-media-on-web#pageresponsiveness) to `"disabled"` to prioritize render speed instead.
