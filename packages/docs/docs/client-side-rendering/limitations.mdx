...
Currently, the `scale`, `rotate`, `translate` and `perspective` transformations are currently <a style={{color: 'red'}}>not supported</a>.

### Backgrounds

The `background-color` property is <a style={{color: 'green'}}>supported</a>.  
The `background-image` property and other `background-*` properties are <a style={{color: 'red'}}>not supported</a>.
...
Nested `border-radius` properties are currently [buggy](https://github.com/remotion-dev/remotion/issues/6057).  
