...
Inside an `<svg>`, a web font is only used if it is passed in through [`svgFonts`](/docs/web-renderer/render-still-on-web#svgfonts). An SVG is drawn by loading it as an image, which cannot see the fonts of the page around it.  
