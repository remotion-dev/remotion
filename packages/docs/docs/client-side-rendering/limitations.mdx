...
:::info
If the optional [HTML-in-canvas capture](/docs/client-side-rendering/html-in-canvas) mode is active, these CSS limitations don't apply because a full screenshot is taken instead.
:::
...
The `background-size` and `background-position` properties are <a style={{color: 'green'}}>supported</a> for linear gradients with pixel or percentage values.<br />
...
The `font-stretch` property is <a style={{color: 'green'}}>supported</a>. Percentage values are approximated to the closest named stretch supported by the Canvas API.
...
The `text-decoration` property is <a style={{color: 'green'}}>supported</a> for `solid`, `dotted`, `dashed`, `double`, and `wavy` `text-decoration-style` values on `underline`, `overline`, and `line-through` lines with `text-decoration-color` and `text-decoration-thickness`.
`wavy` lines are approximated with Bézier curves and may not exactly match the browser-native wave shape. Pixel-perfect line positions are <a style={{color: 'red'}}>not supported</a>.
...
### Web fonts inside SVG

Fonts loaded through [`@remotion/fonts`](/docs/fonts-api) or [`@remotion/google-fonts`](/docs/google-fonts) are embedded automatically when they are used by `<text>`, `<tspan>`, or `<textPath>` inside an `<svg>` from <AvailableFrom v="4.0.525" inline />.

The font selection must be part of the SVG markup, for example through an inline `style`, a `font-family` attribute, or a `<style>` element inside the SVG. A CSS class whose rules are defined outside the SVG is not preserved when the SVG is converted into an image.

Fonts loaded through CSS `@font-face`, a Google Fonts CSS import, or the browser's `FontFace` API are not embedded. Text inside `<foreignObject>` is not detected as SVG text and does not receive embedded fonts.

In Safari, SVG text may intermittently use a fallback font because WebKit can report the SVG image as loaded before its embedded font is ready. Use Chrome or Firefox when reliable SVG web font rendering is required.
...
[`<Img>`](/docs/img) is <a style={{color: 'green'}}>supported</a>, including SVG data URIs.<br />
...
[`<HtmlInCanvas>`](/docs/remotion/html-in-canvas) is <a style={{color: 'green'}}>supported</a> when the component itself is supported, including its [`onPaint`](/docs/remotion/html-in-canvas#onpaint) and [`onInit`](/docs/remotion/html-in-canvas#oninit) callbacks. The optional [HTML-in-canvas capture mode](/docs/client-side-rendering/html-in-canvas) falls back to the built-in DOM composer for such compositions to avoid nesting.<br />
...
By default, [`renderMediaOnWeb()`](/docs/web-renderer/render-media-on-web) tries to free the event loop between expensive render phases to keep the page responsive.
Set [`pageResponsiveness`](/docs/client-side-rendering/page-responsiveness) to `"disabled"` to prioritize render speed instead.

Higher responsiveness settings make renders slower.
