...
The [`toneFrequency`](/docs/audio/pitch) prop of `<Audio>` and `<Video>` from `@remotion/media` is supported from <AvailableFrom v="4.0.523" inline />. Its value must stay constant for each audio or video asset throughout the render.
