...
4. **Use `errorBehavior="ignore"`**: If you want to gracefully skip images that fail to load or are tainted, use the [`errorBehavior`](/docs/img#errorbehavior) prop on `<Img>`:

```tsx
<Img src="https://example.com/maybe-cors-enabled.png" errorBehavior="ignore" />
```

This will render nothing instead of failing the render if the image cannot be drawn to canvas.
