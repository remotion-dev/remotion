...
You can add [third-party Element libraries](/elements/libraries) to Browse Elements in Remotion Studio. Adding a library registers its catalog; installing an individual Element's source code is a separate action.
