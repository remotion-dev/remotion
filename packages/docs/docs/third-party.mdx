...
You can add [third-party Element Libraries](/elements/libraries) to Browse Elements in Remotion Studio. Adding an Element Library registers it; installing an individual Element's source code is a separate action.
...
Use the [`@remotion/gsap`](/docs/gsap) package.
