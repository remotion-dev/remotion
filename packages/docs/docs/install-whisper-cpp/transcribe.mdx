...
<CompatibilityTable chrome={false} firefox={false} safari={false} nodejs bun serverlessFunctions="Native binary required" clientSideRendering="" serverSideRendering="" player="" studio="" hideBrowsers />

Serverless functions need a compatible Whisper.cpp executable, the model, and enough writable storage and execution time for transcription.
