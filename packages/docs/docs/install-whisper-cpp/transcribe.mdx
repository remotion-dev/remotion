...
If you haven't installed Whisper.cpp, you can do so for example through [`installWhisperCpp()`](/docs/install-whisper-cpp/install-whisper-cpp) and use the same [`to`](/docs/install-whisper-cpp/install-whisper-cpp#to) path.

### `whisperCppVersion`

The version of Whisper.cpp that is installed in `whisperPath`, for example `'1.5.5'`. Pass the same [`version`](/docs/install-whisper-cpp/install-whisper-cpp#version) that you passed to [`installWhisperCpp()`](/docs/install-whisper-cpp/install-whisper-cpp).  
From version `1.7.4`, the `build/bin/whisper-cli` executable is used instead of `main`.
