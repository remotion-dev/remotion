...
An [`AbortSignal`](https://developer.mozilla.org/en-US/docs/Web/API/AbortSignal) for canceling transcription. Default: no cancellation signal. Calling `abort()` on its controller stops token generation and rejects the promise with the signal's reason. A GPU operation already in progress must finish first. No partial transcript is returned.
