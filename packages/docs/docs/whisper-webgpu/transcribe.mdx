...
import {downloadWhisperModel, resampleTo16Khz, transcribe} from '@remotion/whisper-webgpu';
...
Called while a previously downloaded model is initialized. Call [`downloadWhisperModel()`](/docs/whisper-webgpu/download-whisper-model) first to download the files and receive download progress.

### signal?<AvailableFrom v="4.0.528" />

An [`AbortSignal`](https://developer.mozilla.org/en-US/docs/Web/API/AbortSignal) for canceling transcription.  
Calling `abort()` on its controller stops token generation and rejects the promise with the signal's reason.  
A GPU operation already in progress must finish first.  
No partial transcript is returned.

```ts twoslash title="cancel-transcription.ts"
import {transcribe} from '@remotion/whisper-webgpu';

declare const channelWaveform: Float32Array;
// ---cut---
