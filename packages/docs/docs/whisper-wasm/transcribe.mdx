...
To transcribe an audio file, you first need to download a model using [`downloadWhisperModel()`](/docs/whisper-wasm/download-whisper-model) and turn the file into a 16kHz `Float32Array` using [`resampleTo16Khz()`](/docs/whisper-wasm/resample-to-16khz).

## Example

```tsx twoslash title="app.ts"
import {resampleTo16Khz} from '@remotion/whisper-wasm';
...
A `Float32Array` representing the mono audio waveform data, resampled to 16kHz. This is typically obtained by calling [`resampleTo16Khz()`](/docs/whisper-wasm/resample-to-16khz) with an audio `File` or `Blob`.

### `model`

The Whisper model to use for transcription (e.g., `'tiny.en'`, `'base'`, `'small'`). This determines the size, speed, and accuracy of the model. Ensure the model has been downloaded using [`downloadWhisperModel()`](/docs/whisper-wasm/download-whisper-model) before calling `transcribe`.
...
- [Source code for `transcribe()`](https://github.com/remotion-dev/remotion/blob/main/packages/whisper-wasm/src/transcribe.ts)
- [`resampleTo16Khz()`](/docs/whisper-wasm/resample-to-16khz)
