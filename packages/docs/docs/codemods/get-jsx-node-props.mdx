...
Prop names for which [`staticFile()`](/docs/staticfile) calls and `new Date()` expressions should be read as serialized asset and date values. Defaults to `[]`; these expressions are otherwise treated as computed.
