...
Describes a JSX element to insert into source code. Pass the result to [`addElement()`](/docs/codemods/add-element) to add it to a composition or next to an existing node, or to [`wrapJsxNode()`](/docs/codemods/wrap-jsx-node) to wrap an existing node.
...
The JSX tag name. Use a component name such as `Solid`, a member expression such as `Interactive.Div`, or an HTML tag such as `div`.
...
A record of prop values. Strings become string attributes, other values become JSX expressions. Values must be strings, finite numbers, booleans, `null`, arrays, or plain objects. Use [`staticFileValue()`](/docs/codemods/static-file-value) to reference a file in the public folder. Defaults to `{}`.
...
### component, importPath, props, children

The values passed to `createElement()`, with `importPath` defaulting to `null`. `JSON.stringify()` serializes an element to a plain description that `createElement()` accepts again.
...
- [`wrapJsxNode()`](/docs/codemods/wrap-jsx-node)
