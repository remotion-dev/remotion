...
Describe JSX with [`createElement()`](/docs/codemods/create-element) and insert it with [`addElement()`](/docs/codemods/add-element). Elements are immutable descriptions: methods such as `withProp()` and `withChild()` return new elements. Imports and local names are resolved when the element is inserted.
...
Elements can also be inserted `inside`, `before`, or `after` an existing node, and [`wrapJsxNode()`](/docs/codemods/wrap-jsx-node) wraps an existing node in an element.
...
import {applyCodemodChanges, getJsxNodes, getJsxNodeProps, updateJsxNodeProps, type CodemodProject} from '@remotion/codemods';
...
The `SequenceNodePath` identifying the element in the current source snapshot. Obtain it from [`getJsxNodes()`](/docs/codemods/get-jsx-nodes), an insertion result, or a mounted layer's `nodePathInfo.sequenceSubscriptionKey.nodePath`. Treat its contents as opaque. To traverse the tree, compare it with the `parentNodePath` that [`getJsxNodes()`](/docs/codemods/get-jsx-nodes) returns for each node.
