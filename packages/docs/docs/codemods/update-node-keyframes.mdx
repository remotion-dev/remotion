---
image: /generated/articles-docs-codemods-update-node-keyframes.png
title: 'updateNodeKeyframes()'
crumb: '@remotion/codemods'
---

# updateNodeKeyframes()<AvailableFrom v="4.0.530" />
...
```ts twoslash title="update-node-keyframes.ts"
import {updateNodeKeyframes, type CodemodProject, type NodeReference} from '@remotion/codemods';
...
Creating an animation that needs a new [`useCurrentFrame()`](/docs/use-current-frame) hook requires an enclosing function. Existing supported interpolation expressions can be edited outside functions. Static prop edits outside functions remain supported by [`updateNodeProps()`](/docs/codemods/update-node-props).
...
- [Source code for this function](https://github.com/remotion-dev/remotion/blob/main/packages/codemods/src/update-node-keyframes.ts)
