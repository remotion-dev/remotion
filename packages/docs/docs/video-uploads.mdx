...
:::note
The `canDecode()` function checks if a video can be loaded into [`<Video>`](/docs/media/video) from `@remotion/media`. Note that [`<Html5Video>`](/docs/media/html5-video) and [`<OffthreadVideo>`](/docs/offthreadvideo) have different format compatibilities. Learn more about the supported media formats: [Video formats](/docs/miscellaneous/video-formats)
:::
