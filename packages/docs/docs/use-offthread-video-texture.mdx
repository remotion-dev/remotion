...
import {getRemotionEnvironment, staticFile, useRemotionEnvironment, useVideoConfig, Video} from 'remotion';
...
const My3DVideo = () => {
  ...
  return (
    <>
      {env.isRendering ? null : <Video ref={videoRef} src={videoSrc} style={{position: 'absolute', opacity: 0}} />}
      <ThreeCanvas width={width} height={height}>
        <mesh>{videoTexture ? <meshBasicMaterial map={videoTexture} /> : null}</mesh>
      </ThreeCanvas>
    </>
