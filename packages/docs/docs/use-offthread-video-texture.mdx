...
<CompatibilityTable chrome="During rendering" firefox={false} safari={false} nodejs="" bun="" serverlessFunctions="" clientSideRendering={false} serverSideRendering player={false} studio={false} hideServers />

Only works during server-side rendering. Use [`useVideoTexture()`](/docs/use-video-texture) for preview, or [`@remotion/media`](/docs/videos/as-threejs-texture) for a texture that also supports client-side rendering.
