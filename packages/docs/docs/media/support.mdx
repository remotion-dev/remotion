...
If a media file cannot be decoded, be it because of an unsupported codec or because it does not support CORS, then we plan on automatically falling back to [`<OffthreadVideo>`](/docs/offthreadvideo) if available. This is currently not yet implemented.
