...
If a media file cannot be decoded, be it because of an unsupported codec or because it does not support CORS, then `@remotion/media` automatically falls back to [`<OffthreadVideo>`](/docs/offthreadvideo) or [`<Audio>`](/docs/audio) from the [`remotion`](/docs/remotion) package.

See: [Fallback from `@remotion/media` to `<OffthreadVideo>` or `<Audio>`](/docs/media/fallback)
