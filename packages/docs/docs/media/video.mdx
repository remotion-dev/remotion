...
### `fetchCache?`<AvailableFrom v="4.0.465" />

Controls the [`cache`](https://developer.mozilla.org/en-US/docs/Web/API/Request/cache) option of the `fetch()` requests made to retrieve the video data.

Accepts `"default"` (default behavior of `fetch()`), `"no-store"`, `"reload"`, `"no-cache"`, `"force-cache"` or `"only-if-cached"`.
Set to `"no-store"` if a CDN or browser cache returns invalid range request responses.
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <Video fetchCache="no-store" src="https://example.com/video.mp4" />
    </AbsoluteFill>
  );
};
```
