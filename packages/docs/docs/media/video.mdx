...
Updates after mount are ignored, so inline objects do not recreate the media source every render.
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <Video requestInit={{cache: 'no-store'}} src="https://example.com/video.mp4" />
    </AbsoluteFill>
  );
};
```
