...
### `effectsOutputSize?`<AvailableFrom v="4.0.513" />

Sets the width and height of the canvas on which [`effects`](#effects) are evaluated. By default, the encoded video dimensions are used.

Use this prop to keep pixel-based effects consistent when the same video is available at different resolutions, for example when switching between a proxy and an original file.
...
export const MyVideo = () => {
  return <Video src={staticFile('video.webm')} effects={[pixelate({blockSize: 32})]} effectsOutputSize={{width: 1920, height: 1080}} />;
};
```

The values are rounded to the nearest pixel and must be positive and finite. This prop has no effect if `effects` is empty.

Without `effectsOutputSize`, the same effect has a different scale on 512px and 2048px encodes:

<img src="/img/media/video-effects-output-size-before.png" alt="The same pixelate effect has different block sizes on two encodes of the same video" />

Setting `effectsOutputSize` to 1024 × 1024 gives both encodes the same effect scale:

<img src="/img/media/video-effects-output-size-after.png" alt="The same pixelate effect has matching block sizes after setting effectsOutputSize" />
...
Use this prop to let the video buffer before it becomes visible. See [Premounting](/docs/player/premounting).
...
Changing `playbackRate` also changes the pitch of the video's audio. Pitch-preserving speed changes are not supported by the Mediabunny and WebCodecs path. If the video [falls back to `<OffthreadVideo>`](/docs/media/fallback) during preview or server-side rendering, audio pitch is preserved by default.
:::
...
Useful for [video manipulation](/docs/videos/video-manipulation).
...
The value can be changed while previewing. It must stay constant across the composition timeline during rendering.

Works during preview and server-side rendering. It is not supported during [client-side rendering](/docs/client-side-rendering/).

If the component [falls back to `<OffthreadVideo>`](/docs/media/fallback) during preview, pitch shifting is not applied.
