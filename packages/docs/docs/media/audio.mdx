...
[`<Audio>` from `@remotion/media`](/docs/media/audio) is the recommended component for embedding audio in Remotion.

During rendering, it extracts the exact audio using [Mediabunny](/docs/mediabunny) instead of FFmpeg. This keeps the audio in sync with Remotion's timeline.
...
### `durationInFrames?`<AvailableFrom v="4.0.446" />

Selects this many source frames starting at `trimBefore`. The clip occupies `durationInFrames / playbackRate` frames on the parent timeline, or uses this range as its loop when `loop` is enabled. Same meaning as [`durationInFrames`](/docs/sequence#durationinframes) on [`<Sequence>`](/docs/sequence).
...
Mounts the audio for the specified number of frames before its `from` frame, allowing it to buffer before playback starts. See [Premounting](/docs/player/premounting).

### `postmountFor?`<AvailableFrom v="4.0.495" />

Keeps the audio mounted for the specified number of frames after its duration has ended.
...
By passing `durationInFrames={60}`, the audio plays for 60 source frames.

The audio will play the range from `00:02:00` to `00:04:00`, meaning the audio will play for 2 seconds.

For exact behavior, see [Timing and trimming](/docs/timing).
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <Audio src={staticFile('audio.mp3')} trimBefore={60} durationInFrames={60} />
    </AbsoluteFill>
  );
};
```
...
:::info Deprecated
Use [`durationInFrames`](#durationinframes) instead. See [PR #11685](https://github.com/remotion-dev/remotion/pull/11685).
:::

Removes a portion of the audio at the end (right side). If `durationInFrames` ends the range earlier, the shorter range is used. See [`trimBefore`](#trimbefore) for an explanation.
...
:::note
Playing audio in reverse is not supported.

Changing `playbackRate` also changes the audio pitch. Pitch-preserving speed changes are not supported by the Mediabunny and WebCodecs path. If the audio [falls back to `<Html5Audio>`](/docs/media/fallback) during preview or server-side rendering, audio pitch is preserved by default.
:::
...
Works during preview and server-side rendering. [Client-side rendering](/docs/client-side-rendering/) is supported from <AvailableFrom v="4.0.523" inline />.

If the component [falls back to `<Html5Audio>`](/docs/media/fallback) during preview, pitch shifting is not applied.
...
<CompatibilityTable chrome firefox safari nodejs="" bun="" serverlessFunctions="" clientSideRendering="Supported codecs" serverSideRendering player studio hideServers />

See [supported media](/docs/media/support). Unsupported codecs may use a [fallback](/docs/media/fallback) during preview and server-side rendering; client-side rendering requires a decodable source and cannot fall back.
