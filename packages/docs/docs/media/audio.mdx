...
Maps to [`<Audio />` -> `delayRenderRetries`](/docs/audio#delayrenderretries)

#### `delayRenderTimeoutInMilliseconds?`

Maps to [`<Audio />` -> `delayRenderTimeoutInMilliseconds`](/docs/audio#delayrendertimeoutinmilliseconds)

#### `pauseWhenBuffering?`

Maps to [`<Audio />` -> `pauseWhenBuffering`](/docs/audio#pausewhenbuffering)
