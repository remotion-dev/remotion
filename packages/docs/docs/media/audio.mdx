...
Controls the [`cache`](https://developer.mozilla.org/en-US/docs/Web/API/Request/cache) option of the `fetch()` requests made to retrieve the audio data.
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <Audio fetchCache="no-store" src="https://example.com/audio.mp3" />
    </AbsoluteFill>
  );
};
```
