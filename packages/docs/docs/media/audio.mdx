...
Controls whether pitch is preserved when [`playbackRate`](#playbackrate) is not `1`.
Default for embedded audio: `false`.

Starting from <AvailableFrom v="4.0.490" inline />, this option affects preview and rendering. When falling back to [`<Html5Audio>`](/docs/html5-audio), its browser-compatible default remains `true`. Set this option or [`fallbackHtml5AudioProps.preservePitch`](#fallbackhtml5audioprops) to override it.
