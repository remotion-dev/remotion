...
:::warning
**Very experimental**: This component is in a very early stage and does not support some basic features such as volume or playback rate.
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <Audio src="https://parser.media/audio.wav" />
    </AbsoluteFill>
  );
};
```
...
// ---cut---
export const MyComposition = () => {
  return <Audio loop src="https://parser.media/audio.wav" />;
};
```
...
// ---cut---
export const MyComposition = () => {
  return <Audio muted src="https://parser.media/audio.wav" />;
};
```

### `acceptableTimeShiftInSeconds?`

**Pending removal**: Future iterations will also have client-side playback and not support this prop which was designed for the `<audio>` tag.
