...
To prevent [`<Audio>`](/docs/media/audio) from falling back to [`<Audio>`](/docs/audio) from [`remotion`](/docs/remotion), set the [`disallowFallbackToHtml5Audio`](/docs/media/disallowfallbacktohtml5audio) prop:
