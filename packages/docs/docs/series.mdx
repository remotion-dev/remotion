...
Changes the speed of the children. Defaults to `1` and must remain constant across frames. See [`playbackRate`](/docs/sequence#playbackrate) on [`<Sequence>`](/docs/sequence).
The sequence's duration and the next sequence's start remain unchanged. Setting `playbackRate` on `<Series>` speeds up the whole series, including transitions between its sequences.
