...
Either `"absolute-fill"` or `"none"` _(default)_. If set to `"absolute-fill"`, the series is wrapped in an [`<AbsoluteFill>`](/docs/absolute-fill) container.
...
For how many frames of the child timeline the sequence should be displayed. Children are unmounted if they are not within the time range of display. The next sequence starts after `durationInFrames / playbackRate` frames.

To loop within a series scene, put a looping `<Sequence>` inside `<Series.Sequence>`.
...
Changes the speed of the children. Defaults to `1` and must remain constant across frames. The next sequence starts after `durationInFrames / playbackRate` frames. See [`playbackRate`](/docs/sequence#playbackrate) on [`<Sequence>`](/docs/sequence).

Setting `playbackRate` on `<Series>` speeds up the whole series, including transitions between its sequences.
