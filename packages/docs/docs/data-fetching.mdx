...
<Step>2</Step> Ensure you are entitled to a high request rate without running
into a rate limit.
<br />
<Step>3</Step> The data returned by the API must be the same on all threads,
otherwise <a href="/docs/flickering">flickering</a> may occur. <br />
<Step>4</Step> Make sure to not have <code>frame</code> as a dependency of the
<code>useEffect()</code>, directly or indirectly, otherwise data will be fetched
every frame leading to slowdown and potentially running into rate limits. <br />
