...
Multiple frame ranges are not supported. Use local [`renderMedia()`](/docs/renderer/render-media) to render discontinuous sections.
...
For serverless functions, use the [light client](/docs/cloudrun/light-client).
