...
Use this prop to let the animated image load, initialize its decoder, and decode its initial frame before it becomes visible. See [Premounting](/docs/player/premounting).
...
Options that are passed to the internal [`fetch()`](https://developer.mozilla.org/en-US/docs/Web/API/fetch) call when loading the animated image. This can be used to pass credentials or headers for authenticated animated images.

Remotion manages the internal `signal` option itself, so passing a `signal` in `requestInit` has no effect.

```tsx
<AnimatedImage
  src="https://example.com/authenticated-image.webp"
  requestInit={{
    credentials: 'include',
  }}
/>
...
`<AnimatedImage>` inherits [`from`](/docs/sequence#from), [`durationInFrames`](/docs/sequence#durationinframes), [`trimBefore`](/docs/sequence#trimbefore)<AvailableFrom v="4.0.482" inline />, [`name`](/docs/sequence#name), [`showInTimeline`](/docs/sequence#showintimeline) and [`hidden`](/docs/sequence#hidden) from [`<Sequence>`](/docs/sequence).
