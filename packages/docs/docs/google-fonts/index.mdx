...
You can pass a style (such as `normal`, `italic`) to only load that style. If you want multiple styles, call `loadFont()` multiple times.
...
```tsx twoslash title="Load a specific style with limited weights and subsets"
import * as Montserrat from '@remotion/google-fonts/Montserrat';
...
If you use text measurement APIs, wrap your content in a component that waits for your fonts before rendering:

- [Higher-order component example](/docs/layout-utils/best-practices#example-with-high-order-component)
- [Troubleshooting font loading errors](/docs/troubleshooting/font-loading-errors)
