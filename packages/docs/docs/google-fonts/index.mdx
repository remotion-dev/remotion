...
One way of [embedding fonts](/docs/fonts) in Remotion is `@remotion/google-fonts`, sourcing fonts from [Google Fonts](https://fonts.google.com/).
...
Call [`loadFont()`](/docs/google-fonts/load-font) to start the loading process. In Remotion 4.0, calling it without arguments loads every style, weight and subset, which can cause unnecessary requests. From [v5.0](/docs/5-0-migration), `weights` and `subsets` are required.
