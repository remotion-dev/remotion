...
:::warning
In Remotion 4.0, calling `loadFont()` without arguments loads all styles, weights and character subsets of a font. This can trigger a lot of network requests and may lead to warnings or timeouts.

From [v5.0](/docs/5-0-migration), you must pass non-empty `weights` and `subsets` arrays in the `options` argument. Otherwise `loadFont()` throws.
:::
...
Use TypeScript autocompletion or [`getInfo`](/docs/google-fonts/get-info) to discover available options.

The `style` argument is optional. In Remotion 4.0, if no arguments are passed, all styles, weights and subsets are loaded.

From [v5.0](/docs/5-0-migration), `options.weights` and `options.subsets` are required and must be non-empty arrays.
...
Array of weights that should be loaded. Optional in v4.x. Required from v5.0.

#### subsets?

Array of font subsets that should be loaded. Optional in v4.x. Required from v5.0.
