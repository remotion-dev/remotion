...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <Video src={staticFile('video.webm')} trimLeft={60} trimRight={120} />
    </AbsoluteFill>
  );
};
```
...
Removes a portion of the video at the end (right side). See [`trimLeft`](/docs/video#trimleft) for an explanation.
