...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <Video audioStreamIndex={1} src={'https://parser.media/multichannel-audio.mov'} />
    </AbsoluteFill>
  );
};
```
...
[`<OffthreadVideo>`](/docs/offthreadvideo) is a drop-in alternative to `<Video>`. To decide which tag to use, see: [Comparison of video tags](/docs/video-tags)
