...
Also supports adding auxiliary packages such as `zod`, `mediabunny`, and `@huggingface/transformers` with the recommended version for your Remotion installation.
...
1. Verify that all package names are valid Remotion packages or supported auxiliary packages
