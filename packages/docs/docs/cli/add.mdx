...
When adding multiple packages, the command will install all of them in a single operation with the same version number, making it efficient to set up multiple dependencies at once.
