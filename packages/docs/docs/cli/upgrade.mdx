...
Installed auxiliary packages such as `zod`, [`mediabunny`](/docs/mediabunny/version), and [`@huggingface/transformers`](/docs/whisper-webgpu) are also upgraded to the versions recommended for the new Remotion version.
