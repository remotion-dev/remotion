...
If you are rendering on [Remotion Lambda](/docs/lambda), the supported codec set is different. Lambda ships with the essential codecs to keep the runtime lean, so `av1` is not available there. See [`npx remotion lambda render`](/docs/lambda/cli/render#--codec).
