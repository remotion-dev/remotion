...
If you are rendering on [Remotion Lambda](/docs/lambda) or on Linux ARM64 GNU, `av1` is not available because those runtimes ship only the essential codecs to stay lean. See [`npx remotion lambda render`](/docs/lambda/cli/render#--codec).
