...
By default, if a Remotion Studio is already running on the target port, the CLI will try to focus the existing browser tab or open a new tab if none is found.
