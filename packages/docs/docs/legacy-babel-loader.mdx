...
When using the [`bundle()`](/docs/bundle) Node.js API, provide the Webpack override directly because Node.js APIs do not read from the config file. Pass the resulting directory to [`deploySite()`](/docs/lambda/deploysite) when deploying to Lambda.
