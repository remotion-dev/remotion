...
When using the [`bundle()`](/docs/bundle) Node.js API, provide the Webpack override directly because Node.js APIs do not read from the config file. To deploy the resulting directory to Lambda, pass it to [`deploySiteFromBundle()`](/docs/lambda/deploysitefrombundle).
