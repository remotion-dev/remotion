...
# Rendering GIFs<AvailableFrom v="3.1" />
...
- Assuming `everyNthFrame` is `2`, only every 2nd frame is rendered: `0`, `2`, `4`, `6` and so on. A 30FPS video would now become a 15FPS GIF.
- If `everyNthFrame` is `3`, only every 3rd frame is rendered: `0`, `3`, `6`, `9` and the pattern continues.

The first frame is always included. When using a custom `frameRange`, `everyNthFrame` offsets from the start of the range.
