...
The the example above, the [`using`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/using) keyword is used to automatically close the `VideoSample` and `Ìnput` objects when they goes out of scope. Make sure to keep no references to them to ensure proper cleanup.
...
```ts title="Fail if not able to extract within 10 seconds"
const controller = new AbortController();
