...
export const getVideoFrameRate = async (
  ...
  const toleranceInSeconds = 1 / 1000;
  const intervalSpread = Math.max(...intervals) - Math.min(...intervals);
  const floatingPointTolerance =
    Number.EPSILON *
    Math.max(
      1,
      ...sortedTimestamps.map((timestamp) => Math.abs(timestamp)),
    ) *
    2;
  const isConstant =
    intervalSpread <= toleranceInSeconds + floatingPointTolerance;
...
- Video timestamps have limited precision. For example, Matroska timestamps commonly have millisecond precision, leading to uneven timestamps such as `33 ms`, `67 ms`, and `100 ms`.
  We still consider this a constant frame rate video, so we allow the intervals to vary by one millisecond.
