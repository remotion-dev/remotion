...
- `InteractiveThree.Group` registers a React Three Fiber group in the Studio timeline and exposes its local X, Y, and Z position in the Inspector. Select the group in the timeline to edit its position or add keyframes. This prototype does not support selecting the mesh by clicking the preview.

```tsx
import {InteractiveThree, ThreeCanvas} from '@remotion/three';

<ThreeCanvas width={1280} height={720}>
  <InteractiveThree.Group name="Product" positionX={0} positionY={0} positionZ={0}>
