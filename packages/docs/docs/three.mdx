...
- `InteractiveThree.Group` registers a React Three Fiber group in the Studio timeline. Its local position, rotation in degrees, and scale can be edited in the Inspector or keyframed. Select the group in the timeline or click its mesh in the preview, then use the Move, Rotate, or Scale handles to edit it directly. Studio saves each drag to the source file as one undoable action. Scale handles change all three axes proportionally by default. A small editor-only guide shows the group's position five frames earlier.
...
- [`<ThreeWebGPUCanvas />`](/docs/three-webgpu-canvas) uses Three.js's `WebGPURenderer` and supports TSL node materials.

- [`useVideoTexture()`](/docs/use-video-texture) allows you to use a Remotion [`<Html5Video />`](/docs/html5-video) as a texture map.

- [`useOffthreadVideoTexture()`](/docs/use-offthread-video-texture) is an equivalent to [`useVideoTexture()`](/docs/use-video-texture), but displays the exact frame as an `Three.ImageTexture` during rendering, similarly to [`<OffthreadVideo />`](/docs/offthreadvideo).

For everything else, you can use the standard [React Three Fiber](https://github.com/pmndrs/react-three-fiber) APIs.
