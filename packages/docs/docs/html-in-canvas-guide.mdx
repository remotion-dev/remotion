...
Nesting [`<HtmlInCanvas>`](/docs/remotion/html-in-canvas) components is not supported and throws an error. If you need to combine multiple effects, merge them into a single `onPaint` callback.
...
Not directly related, but [`@remotion/web-renderer`](/docs/client-side-rendering/html-in-canvas) can also use this technology to capture frames.

This is opt-in because the mode cannot capture compositions containing [`<HtmlInCanvas>`](/docs/remotion/html-in-canvas): that would create a nested HTML-in-canvas subtree.

## Nested HTML-in-canvas

Nesting `<HtmlInCanvas>` components is unsupported in preview, client-side rendering, and server-side rendering.
