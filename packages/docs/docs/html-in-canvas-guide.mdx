...
<SuggestedPrompt prompt="/remotion-markup Create a sample composition and apply an HTML-in-canvas glitch effect to it." />

<SuggestedPrompt
  prompt={
    '/remotion-markup Make a new composition with a subtle CRT convex-shape shader over an HTML element. Use a terminal and Remotion theme. Animate the command and output of "npx create-video@latest --yes --blank my-video", followed by invoking "claude" and entering the prompt "Add a CRT effect using HTML-in-canvas".'
  }
/>
