...
4. Options object with the same keys as [`interpolate()`](/docs/interpolate#easing) (`extrapolateLeft`, `extrapolateRight`, `easing`, `posterize`), including a [per-segment `easing` array](/docs/interpolate#per-segment-easing-array) when you have more than two keyframes. Optional.
...
When using the generated styles for [client-side rendering](/docs/client-side-rendering), the [supported CSS properties](/docs/client-side-rendering/limitations) still apply.
