...
Basic controls are inferred from the values in `defaultProps`. You do not need to define a [`schema`](/docs/composition#schema).
...
If no `schema` is specified, Remotion infers controls for:
...
- Dates
- Nested objects
- Colors, if [`@remotion/zod-types`](/docs/zod-types) is installed
- [`staticFile()`](/docs/staticfile) assets
- Non-empty arrays whose items have the same inferred type

Fields named `type` are treated as literals and cannot be edited. This prevents changing a discriminator without defining the other possible object shapes.

`null`, `undefined`, empty arrays, arrays with mixed item types and values whose type cannot be inferred do not get a visual control. Define an explicit schema if these values should be editable.

`zod` must be installed for Remotion to infer controls:
...
Define a [Zod schema](/docs/schemas) when you need validation, constraints or specialized controls. An explicit schema takes precedence over the controls inferred from `defaultProps`.
