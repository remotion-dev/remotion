...
You can edit the [`defaultProps`](/docs/composition#defaultprops) of a [`<Composition>`](/docs/composition) visually in the Remotion Studio.

Basic controls are [inferred from the values in `defaultProps`](/docs/default-props-inference). You do not need to define a [`schema`](/docs/composition#schema).
...
icon in the top right corner of the Remotion Studio to open the right sidebar. Alternatively, press <kbd>Cmd/Ctrl</kbd> + <kbd>J</kbd> to toggle the sidebar. The composition's default props are in the Default Props section.

## Inspector sections<AvailableFrom v="4.0.526" />

Click a section title, such as Metadata, Default Props, or Actions, to collapse or expand its contents. You can also focus the title with <kbd>Tab</kbd> and press <kbd>Enter</kbd> or <kbd>Space</kbd>. Collapsing Default Props preserves the editor mode and unfinished JSON.

Empty sections, such as Effects without any effects, are not collapsible. Header actions remain separate from the title: adding an effect or importing captions opens the corresponding section.
...
If no `schema` is specified, Remotion infers controls from `defaultProps`. See [Inferring controls from default props](/docs/default-props-inference) for the supported values and edge cases.

### Defined by a schema

See [When to define a schema](/docs/default-props-inference#when-to-define-a-schema) for cases where an explicit [Zod schema](/docs/schemas) is useful. An explicit schema takes precedence over the controls inferred from `defaultProps`.
