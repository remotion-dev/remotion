...
If no `schema` is specified, Remotion infers controls from `defaultProps`. See [Inferring controls from default props](/docs/default-props-inference) for the supported values, edge cases and installation requirements.
