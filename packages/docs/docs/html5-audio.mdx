...
### `durationInFrames?`<AvailableFrom v="4.0.530" />

Selects this many source frames starting at `trimBefore`. The audio occupies `durationInFrames / playbackRate` frames on the parent timeline, or uses this range as its loop when `loop` is enabled.
...
Removes a portion of the audio at the beginning (left side).
...
export const MyVideo = () => {
  return (
    <AbsoluteFill>
      <Html5Audio src={staticFile('audio.mp3')} trimBefore={60} durationInFrames={60} />
    </AbsoluteFill>
  );
};
```
...
Removes a portion of the audio at the end (right side).

### ~~`startFrom?` / `endAt?`~~

:::info Deprecated
These props have been renamed to [`trimBefore`](#trimbefore) and [`trimAfter`](#trimafter) in 4.0.319. They will continue to work but you cannot use them together with the new props.
:::
...
`preservePitch` only affects preview playback. During server-side rendering, [`playbackRate`](#playbackrate) always preserves the audio pitch, regardless of this prop. Use [`toneFrequency`](/docs/audio/pitch) to change the pitch during server-side rendering.
...
Adjust the pitch of the audio during server-side rendering. This prop is not supported during preview.
