...
- Separate video assets into background and foreground layers
...
### `install_package`<AvailableFrom v="4.0.523" />
...
### `transcribe_asset`<AvailableFrom v="4.0.523" />

Transcribes an audio or video asset from the `public/` folder and adds the work to the Jobs queue. The output is a JSON file containing a [`Caption[]`](/docs/captions/caption) array.
...
The default model is `small.en`. The default output is `<asset>-captions.json`. See [`transcribe()`](/docs/whisper-webgpu/transcribe) for the Whisper options and their defaults.
...
```ts title="Missing package output"
{
  success: false;
  error: string;
  installPackage: {
    tool: 'install_package';
    packageName: '@remotion/whisper-webgpu';
  }
}
```
...
### `separate_video_layers`<AvailableFrom v="4.0.523" />

Separates a video asset from the `public/` folder into background and foreground WebM files and adds the work to the Jobs queue.
...
```ts title="Input"
{
  ...
  model?: string;
  audio?: 'base' | 'foreground' | 'both' | 'none';
  videoBitrate?:
    | 'very-low'
    | 'low'
    | 'medium'
...
The default model is `ben2-base`, audio is kept in the background layer, and video quality defaults to `very-high`. Output names default to `<asset>-base.webm` and `<asset>-foreground.webm`.
...
```ts title="Missing package output"
{
  ...
  installPackage: {
    tool: 'install_package';
    packageName: '@remotion/video-matting';
  }
}
```
...
- "Separate `product-shot.mp4` into background and foreground layers."
