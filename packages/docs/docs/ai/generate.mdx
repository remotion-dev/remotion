...
Having more properties and metadata around the code you generated is beneficial. Classic properties go from reasoning, FPS, width to height.
...
The structured output guarantees you receive valid JSON with all required fields, making it easier to configure the Player or rendering pipeline. You can also go beyond just the type and enter result ranges like for the FPS where we would expect the result to be between 1 and 120.  
...
As a human, we wouldnt read the whole documentation before we start implementing. We would take a look at the problem and think what we need to solve it. Then we would only read the docs for the relevant tools. THis is exactly what the Skills strategy is all about.
