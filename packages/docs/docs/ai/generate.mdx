...
This simple system prompt instructs the model about the request and its purpose. It relies entirely on the model's existing knowledge of Remotion - not ideal, but a good starting point.
...
Note that the raw output often includes markdown code fences (like ` ```jsx `), which require additional prompting or sanitation to remove.

## Structured Output with Zod

Adding metadata alongside the generated code is useful. Common properties include reasoning, FPS, width, and height.
...
With the basics covered, the next step is improving output quality. Finding the perfect system prompt is complex. Depending on what you want to build, you could optimize for specific animation types or even your corporate branding.
...
Be careful of context rot - the degradation in output quality and instruction-following that occurs as context grows. Be selective about what goes into the system prompt.
...
The generated component code could serve as a first draft for an animation or as inspiration for learning Remotion's APIs. To preview it, you need to compile and render it. The [next guide](/docs/ai/dynamic-compilation) shows how to turn this code string into a live preview in the browser.

## See Also

For a Remotion code generation implementation with validation, skill detection, streaming and more, see the [Prompt to Motion Graphics template](https://github.com/remotion-dev/remotion/tree/main/packages/template-prompt-to-motion-graphics):

```bash
npx create-video@latest --prompt-to-motion-graphics
```

- [Dynamic Compilation](/docs/ai/dynamic-compilation)
