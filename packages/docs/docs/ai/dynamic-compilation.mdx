...
Starting with the simplest possible case - a component with no imports, just pure JSX:
...
<details>
<summary>How does the Function constructor work?</summary>
...
The key insight: `React` inside the transpiled code refers to whatever value is passed when calling the function.
...
For a ready-to-use implementation of dynamic compilation and more, see the [Prompt to Motion Graphics template](https://github.com/remotion-dev/remotion/tree/main/packages/template-prompt-to-motion-graphics):
