...
This guide shows how to take Remotion code (a string) and compile it in the browser to display a live preview. No server round-trip required. This is especially useful for AI-generated workflows where the code reuqires dynamic compilation to be shown as it was only just generated and never ran through the build process.

## The Goal

You have a code string from an AI model:
...
export const MyComposition = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "white", justifyContent: "center", alignItems: "center" }}>
      <div style={{ fontSize: 100 }}>{frame}</div>
    </AbsoluteFill>
  );
};
`;
...
And you want to turn it into a live, interactive preview:

```tsx
<Player component={CompiledComponent} durationInFrames={150} fps={30} />
```

This page shows exactly how to bridge that gap.
...
`@babel/standalone` runs Babel entirely in the browser, allowing you to transpile JSX and TypeScript without a build server.
