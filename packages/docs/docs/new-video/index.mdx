...
:::warning
**Experimental Tag**: This tag is currently under development and experimental phase, which means that it can still be used but the API is unstable.
:::

This component imports and displays a video, similar to [`<OffthreadVideo/>`](/docs/offthreadvideo) but during rendering, extracts the exact frame from the video using [`extractFrames()`](/docs/webcodecs/extract-frames) and displays it in a `<canvas>` tag, this ensures that the video is rendered with high accuracy and supports a wide range of codecs more reliably, significantly reducing the [flickering issues](/docs/flickering). 
...
Removes a portion of the video at the end (right side). See [`trimBefore`](/docs/new-video#trimbefore) for an explanation.
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <NewVideo src={staticFile('video.webm')} startFrom={60} endAt={120} />
    </AbsoluteFill>
  );
};
```
...
Removes a portion of the video at the end. See [`startFrom`](/docs/new-video#startfrom) for an explanation.
...
- [Source code for this component](https://github.com/remotion-dev/remotion/blob/main/packages/new-video/src/new-video.tsx)
- [`<Video />`](/docs/video)
