...
- <Step>1</Step> Render each [`<Video>`](/docs/media/video) in a [`<Series.Sequence>`](/docs/series).
- <Step>2</Step> Use [Mediabunny](https://mediabunny.dev) to determine the duration of each video.
...
[`premountFor`](/docs/series#premountfor) mounts each video before it enters the timeline, giving it time to load in the Player. `<Video>` uses the [Player buffer state](/docs/player/buffer-state) by default if more data is needed.
