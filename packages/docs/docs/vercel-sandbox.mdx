...
If you trigger renders through a Vercel Function, the function timeout applies as well.
Depending on function type and plan, this can be lower (for example 800 seconds or 15 minutes), so long renders can still time out before `renderMediaOnVercel()` resolves.

For long renders, avoid awaiting the full render in a request/response cycle.
Use a queue/worker pattern and poll for completion, or choose another SSR option that is better suited for long-running jobs.
