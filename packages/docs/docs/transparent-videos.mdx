...
## Exporting transparent videos by default

Instead of specifying transparency settings via CLI flags or `remotion.config.ts`, you can use [`calculateMetadata`](/docs/dynamic-metadata) to set them as defaults for a specific composition. This approach has several benefits:

- **No CLI flags needed** - Settings are embedded in the composition
- **Works everywhere** - Studio, CLI, and programmatic rendering all use the same defaults
- **Composition-specific** - Different compositions can have different transparency settings
- **Type-safe** - Settings are validated at the composition level

### Setting default codec and pixel format

Use `calculateMetadata` to return `defaultCodec` and configure the render settings:
...
const calculateMetadata: CalculateMetadataFunction<Props> = async ({
  ...
  // Only set transparent defaults if the transparent prop is true
  if (props.transparent) {
    return {
      defaultCodec: 'vp9',
      // Note: imageFormat and pixelFormat must be set via Config or CLI
      // They cannot be set via calculateMetadata
    };
  }
...
### Complete example with transparency

Here's a complete example showing how to set up a composition with transparency as the default:

```tsx twoslash {18-26}
import {Composition} from 'remotion';
import type {CalculateMetadataFunction} from 'remotion';

const MyVideo: React.FC<{transparent: boolean}> = ({transparent}) => {
  return (
    <div
      style={{
...
When rendering, you still need to specify the image format and pixel format:

```bash
remotion render --image-format=png --pixel-format=yuva420p my-video video.webm
```

Or set them in `remotion.config.ts`:
...
The codec will automatically be `vp9` based on the `calculateMetadata` function, so you don't need to specify `--codec=vp9` on the command line.
