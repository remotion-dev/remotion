...
See the [ProRes guide](/docs/prores#transparent-prores) for the full rendering setup.
...
[`<Video>` from `@remotion/media`](/docs/media/video) is recommended for embedding transparent videos.

No `transparent` prop is needed when the video is decoded by [`@remotion/media`](/docs/media). If it falls back to [`<OffthreadVideo>`](/docs/media/fallback), transparency is enabled on the fallback by default.
