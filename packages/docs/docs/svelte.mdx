...
export const PlayerView = forwardRef((props: {data: PlayerSchema; onPaused?: () => void}, ref) => {
  ...
  return <Player ref={playerRef} component={HelloWorld} durationInFrames={150} fps={30} compositionHeight={1080} compositionWidth={1920} inputProps={props.data} style={{width: '100%'}} controls />;
});
```
