...
The API takes care of exchanging messages between the main thread and the Web Worker, so that [`parseMediaOnWebWorker()`](/docs/media-parser/parse-media-on-web-worker) can be used as a drop-in replacement for [`parseMedia()`](/docs/media-parse/parse-media).
