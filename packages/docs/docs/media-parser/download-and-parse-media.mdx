...
<CompatibilityTable chrome="Browser writer" firefox="Browser writer" safari="Browser writer" nodejs bun serverlessFunctions="Writable storage" clientSideRendering="Browser writer" serverSideRendering="Browser writer" player="Browser writer" studio="Browser writer" />

The documented `nodeWriter` requires a writable filesystem. Browser usage requires a [writer](/docs/media-parser/download-and-parse-media#writer) compatible with the browser, such as [`bufferWriter`](/docs/webcodecs/buffer-writer). The [runtime requirements](/docs/media-parser/runtime-support) also apply.
