...
<CompatibilityTable chrome firefox safari nodejs={false} bun serverlessFunctions="Bun workers" clientSideRendering serverSideRendering player studio />

Requires ES modules and a runtime with the browser-style `Worker` API. See [runtime requirements](/docs/media-parser/runtime-support).
