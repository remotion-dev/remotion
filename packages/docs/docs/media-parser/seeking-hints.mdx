...
- Document in mediaParserController() that it can only be attached to 1 at a time
- Test that mediaParserController() cannot be attached to multiple parseMedia() calls at the same time
- Actually use seeking hints
- Document parseMedia() option
- Document convertMedia() option
