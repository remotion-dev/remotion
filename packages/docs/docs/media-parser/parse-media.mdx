...
### `useFixedSizeBuffer?`<AvailableFrom v="4.0.318" />

_number_

Experimental option: Instead of using a resizeable buffer, pre-allocate a fixed size buffer in memory.  
Use this for runtimes that don't support `ArrayBuffer.resize()` (like React Native).
