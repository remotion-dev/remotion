...
<CompatibilityTable chrome={false} firefox={false} safari={false} nodejs bun serverlessFunctions="Filesystem access" clientSideRendering="" serverSideRendering="" player="" studio="" />

Requires a Node.js-compatible filesystem. In serverless functions, the source file must be available in the function filesystem. Use this reader in server code before starting a render.
