...
<CompatibilityTable chrome={false} firefox={false} safari={false} nodejs bun serverlessFunctions="Writable filesystem" clientSideRendering="" serverSideRendering="" player="" studio="" />

Requires a Node.js-compatible filesystem and a writable destination. In serverless functions, use a writable temporary directory provided by the platform.
