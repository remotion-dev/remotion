...
- `sizeInBytes`: The size of the file in bytes.
- `mimeType`: The MIME type of the file.
- `audioType`: The type of the audio file - one of `wav` or `aac`.

:::note
`.mp3` is supported and will not throw an error.  
`@remotion/media-parser` aims to support other audio formats in the future, in which case they will not throw an error but return audio tracks.
:::
