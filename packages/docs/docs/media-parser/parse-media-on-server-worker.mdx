...
<CompatibilityTable chrome={false} firefox={false} safari={false} nodejs={false} bun serverlessFunctions="Bun workers" clientSideRendering="" serverSideRendering="" player="" studio="" />

Requires Bun workers and the ES module entry point. Use it in server code before starting a render, rather than inside a composition.
