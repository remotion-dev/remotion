...
const springTimingWithPause = ({pauseDuration}: {pauseDuration: number}): TransitionTiming => {
  ...
  return {
    getDurationInFrames: ({fps}) => {
      return measureSpring({fps, config: firstHalf}) + measureSpring({fps, config: secondPush}) + pauseDuration;
    },
...
const customTransition = (props: CustomPresentationProps): TransitionPresentation<CustomPresentationProps> => {
...
export const CustomTransition: React.FC = () => {
  ...
  return (
    <TransitionSeries>
      ...
      <TransitionSeries.Transition presentation={slide({direction: 'from-left'})} timing={customTiming({pauseDuration: 10})} />
