...
import {downloadVideoMattingModel, separateVideoLayers} from '@remotion/video-matting';

await downloadVideoMattingModel({model: 'modnet'});
const file = new File([], 'input.mp4');

await using result = await separateVideoLayers({
  src: file,
  model: 'modnet',
  audio: 'base',
  onProgress: (progress) => {
...
In [Node.js](/docs/video-matting/node), strings can also be local file paths, and `file:` URLs are supported.
...
Called while a previously downloaded model is initialized. Default: no callback. Call [`downloadVideoMattingModel()`](/docs/video-matting/download-video-matting-model) first to download the files and receive download progress.
...
#### `base[Symbol.asyncDispose]()`<AvailableFrom v="4.0.528" />

Calls `base.dispose()` when an `await using` scope ends.
...
#### `foreground[Symbol.asyncDispose]()`<AvailableFrom v="4.0.528" />

Calls `foreground.dispose()` when an `await using` scope ends.
...
Disposes both outputs when an `await using` scope ends. Retrieve any blobs before leaving the scope; blobs returned by `getBlob()` remain usable afterward. The loaded model remains in memory for reuse.
...
[Node.js usage](/docs/video-matting/node) is available from <AvailableFrom v="4.0.528" inline />.
