...
Defaults to `false` for compatibility with tolerant parsing. Set to `true` to reject malformed or empty files instead of ignoring unrecognized cues. Strict parsing validates every cue's timestamps (including complete tokens and minute/second ranges), end-before-start errors, and trailing content. Errors include cue and line context. Studio imports use strict parsing.
