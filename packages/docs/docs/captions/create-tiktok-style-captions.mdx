...
- `tokens`: An array of objects, if you want to animate word-per-word:
  ...
  - `lineBreakAfter`: Whether the source caption forced the page to end after this token (_from v4.0.517_).

If a [`Caption`](/docs/captions/caption) has `lineBreakAfter: true`, the current page ends after the corresponding token and the next caption starts a new page. The token preserves the marker, while the page's `text` does not include a trailing newline.
