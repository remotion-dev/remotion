...
This prop also works with `layout="none"`; during premounting, Remotion uses an internal invisible host that does not affect normal playback layout.
...
This prop also works with `layout="none"`.
