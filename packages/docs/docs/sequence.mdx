...
`trimBefore` is measured in the children's frames and is applied after the speed change:
...
A React ref pointing to the element that Remotion Studio should use for drawing the selection outline in the preview.

By default, the Studio outlines the wrapper created by `<Sequence>`. With `layout="none"`, it outlines the rendered element or the bounds of multiple rendered roots. Pass `outlineRef` when you want to outline one specific element instead.
...
export const MyVideo = () => {
  ...
  return (
    <Sequence layout="none" outlineRef={outlineRef}>
      <div ref={outlineRef}>Included in the outline</div>
      <div>Excluded from the outline</div>
    </Sequence>
  );
};
```
