...
For how many frames of the child timeline the sequence should be displayed, starting at [`trimBefore`](#trimbefore). By default it will be `Infinity` to avoid limiting the duration of the sequence.

The sequence occupies `durationInFrames / playbackRate` frames in its parent. When `loop` is enabled, `durationInFrames` is the loop length and the sequence repeats until its parent ends.
...
Repeats the child range selected by [`trimBefore`](#trimbefore) and [`durationInFrames`](#durationinframes) until the parent ends. Default: `false`.
...
export const MyVideo = () => {
  return (
    <Sequence durationInFrames={90}>
      <Sequence trimBefore={30} playbackRate={2} durationInFrames={30} loop>
        <Child />
      </Sequence>
    </Sequence>
  );
