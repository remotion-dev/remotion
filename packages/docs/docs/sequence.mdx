...
Ends the child timeline at the specified frame, measured in the same clock as [`trimBefore`](#trimbefore).
Children receive frames from `trimBefore` up to, but not including, `trimAfter`.

The sequence unmounts after `(trimAfter - trimBefore) / playbackRate` frames in the parent timeline, unless [`durationInFrames`](#durationinframes) or the parent ends it earlier.
Must be greater than `trimBefore`.
...
export const MyVideo = () => {
  return (
    <Sequence from={30} trimBefore={15} trimAfter={45}>
      <Child />
    </Sequence>
  );
};
...
In the example above, `Child` receives frames `15` through `44` and the sequence is visible from composition frame `30` through `59`.

### `loop?`

Repeats the child range selected by [`trimBefore`](#trimbefore) and [`trimAfter`](#trimafter). Default: `false`.

Requires a finite `trimAfter`, because a sequence has no intrinsic duration.
Each cycle lasts `(trimAfter - trimBefore) / playbackRate` frames in the parent timeline.
[`durationInFrames`](#durationinframes) limits the total length; without it, the sequence loops until its parent ends.
...
export const MyVideo = () => {
  return (
    <Sequence trimBefore={30} trimAfter={60} playbackRate={2} durationInFrames={90} loop>
      <Child />
    </Sequence>
  );
};
...
[`Loop.useLoop()`](/docs/loop#useloop) returns the current iteration inside a looping sequence.
To repeat a duration measured in the parent timeline, use [`<Loop>`](/docs/loop).

### `playbackRate?`<AvailableFrom v="4.0.528" />

Changes the speed of the children, including their animations and media. Default: `1`.
Use `2` for double speed or `0.5` for half speed. The value must be positive and finite, and must remain constant across frames.

`from` and `durationInFrames` are measured in the parent timeline. Changing `playbackRate` does not change when the sequence mounts or unmounts.
`trimBefore` and `trimAfter` are measured in the children's frames and are applied after the speed change:
...
At composition frame `30`, `Child` receives frame `10`. At frame `40`, it receives frame `30`: `(40 - 30) * 2 + 10`.
The sequence remains visible through composition frame `89`.

Nested playback rates multiply. A sequence with `playbackRate={2}` inside one with `playbackRate={0.5}` plays at the original speed.
A media component's own `playbackRate` also multiplies with its parent sequences' rates.

The rate applies to [`useCurrentFrame()`](/docs/use-current-frame) called inside descendant components. Animations calculated in the parent and passed as the sequence's `style` or as child props keep the parent's clock.

See [Order of operations](/docs/timing#order-of-operations) for how the timing props combine.
...
export const MyVideo = () => {
  ...
  return (
    <Sequence cropLeft={cropLeft}>
      <div>Revealed from the left</div>
    </Sequence>
  );
};
```
...
:::info Deprecated
Remotion Studio discovers selection outlines automatically. Remove this prop.
:::

By default, the Studio outlines the wrapper created by `<Sequence>`. With `layout="none"`, it outlines the rendered element or the bounds of multiple rendered roots.
...
### `styleWhilePostmounted?`<AvailableFrom v="4.0.340" />

CSS styles to be applied to the container while the sequence is postmounted.
The `style` is still applied, but `styleWhilePostmounted` can override properties. Defaults to no overrides.
