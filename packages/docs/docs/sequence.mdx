...
The toggle is only shown for `<Sequence>`s for which Remotion can write back to the source file – `<Audio>` and `<Video>` tracks no longer have a visibility toggle.
