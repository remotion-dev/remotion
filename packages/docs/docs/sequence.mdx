...
To freeze multiple portions of a sequence and resume afterwards, see [Freeze portions of a sequence](/docs/miscellaneous/snippets/freeze-portions).
...
:::note
If an inline [`style.borderRadius`](https://developer.mozilla.org/en-US/docs/Web/CSS/border-radius) is set, the cropped rectangle uses the same corner radius. A border radius applied through a CSS class is not inherited by the crop.
:::
