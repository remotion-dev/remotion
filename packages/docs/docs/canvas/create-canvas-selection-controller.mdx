...
Creates a standalone `CanvasSelectionController`. [`createCanvasController()`](/docs/canvas/create-canvas-controller) already creates one at `controller.selection`; use this function when selection has a separate lifetime.
