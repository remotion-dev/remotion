...
- Then:
  ...
  - set the [`forSeamlessAacConcatenation`](/docs/renderer/render-media#forseamlessaaconcatenation) option to `false`.
- Else:
  ...
  - set the [`forSeamlessAacConcatenation`](/docs/renderer/render-media#forseamlessaaconcatenation) option to `true`.
...
- [`videoFiles`](/docs/combine-chunks#videofiles): An array of absolute paths to video chunks, must be in order.
- [`audioFiles`](/docs/combine-chunks#audiofiles): An array of absolute paths to audio chunks, must be in order.
- [`outputLocation`](/docs/combine-chunks#outputlocation): An absolute file path where the combined chunks are stored.
- [`onProgress`](/docs/combine-chunks#onprogress): See documentation for `combineChunks()`.
- [`codec`](/docs/combine-chunks#codec): The final codec of the media. If you want to render `h264`, you should now pass `h264`, not `h264-ts`.
- [`framesPerChunk`](/docs/combine-chunks#framesperchunk): The amount of frames that were rendered per chunk. The `everyNthFrame` option will be applied only afterwards, so don't calculate it in.
- [`fps`](/docs/combine-chunks#fps): Must be the value of `fps` that you got from [`selectComposition`](/docs/renderer/select-composition).
- [`preferLossless`](/docs/renderer/combine-chunks#preferlossless): Pass it here as well if you passed it to [`renderMedia()`](/docs/renderer/render-media#preferlossless).
...
- [`audioCodec`](/docs/combine-chunks#audiocodec): If you passed a custom `audioCodec` to the render routines, then you should also pass it here. Otherwise, should be `null`. If the default for the video codec would have been `aac` and you passed `pcm-16` to the render routine, you can now pass `null` again.
- [`audioBitrate`](/docs/combine-chunks#audiobitrate): The bitrate that you would target. Since you might have rendered
- [`numberOfGifLoops`](/docs/combine-chunks#numberofgifloops): If you are rendering a GIF, you can now set the number of loops (remember that for each chunk it must be `null`)
- [`everyNthFrame`](/docs/combine-chunks#everynthframe): If you passed it to each [`renderMedia()`](/docs/renderer/render-media) call, pass it here again.
