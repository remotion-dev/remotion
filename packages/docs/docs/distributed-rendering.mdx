...
- Set the [`enforceAudioTrack`](/docs/renderer/render-media#enforceaudiotrack) option always to `true`. This ensures that chunks without audio include a silent audio track, because other chunks may contain audio. If the entire render is muted, `muted: true` takes precedence.
