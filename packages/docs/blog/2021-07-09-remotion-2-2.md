...
Us developers use environment variables extensively to manage configuration, secrets and other stuff you don't want to commit to a Github repo. If you write videos with code, environment variables are useful too!

We added support for environment variables from the CLI, using a `.env` file, and allowing you to pass variables via our Node.JS APIs too. [Click here to learn more.](/docs/env-variables)

## `<Freeze />` component

This new core component will freeze all of it's children and make them think that the video is paused at a certain time.

Use it for example to display a still frame from a video:
...
[Read more about the `<Freeze />` component here.](/docs/freeze)

## Video + Audio `playbackRate`

This prop allows you to slow down or speed up video and audio elements! Now you can import a video in normal speed and play it in slow motion or timelapse without rerendering the video.
...
Three.JS is admittedly not easy, but with the work that [Poimandres](https://github.com/pmndrs) is doing in making it more approachable and more integrated with React, this is changing rapidly. Creating 3D videos in React and rendering them to real MP4 videos, we are making it possible!
...
- Fixed a memory leak while calling getCompositions(). (28f86f24)
- Add emoji support to Dockerfile (#476)
- The preview now shows the current time within a sequence in a timeline (d925768f)
- TTF fonts can now be loaded locally
- Fix Remotion Player assuming it's in a rendering environment
- Fix getAudioMetadata() and getAudioDuration() not returning an error in the `onError` event (1ea7b151)
- Added documentation on how to use TailwindCSS
- Fixed documentation about how to use SASS/SCSS
- If you are using a plain JS entry file, you can also use a remotion.config.js config file.
- You can customize how many timeline tracks should be shown at most using `Config.Preview.setMaxTimelineTracks()`
- Fixed a bug where environment variables would not be immediately accessible
- Input props flag can now also be passed for preview
- Better input validation of the most common functions
- Improve rendering of very long videos, using less memory and avoiding a Maximum callstack exceeded error message.
- Setting `crf` to 0 is not allowed anymore for H264 videos as it will lead to a video that does not play on all platforms.
- The docs for `<Sequence />` now have interactive examples.
- A lot of tests have been added to the core functions.
- The Typescript types have been improved to allow for more types of components to be passed to `component`.
