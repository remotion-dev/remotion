...
const DEFAULT_AUDIO_PATH = './public/podcast-audio.wav';
...
async function askQuestions(rl: readline.Interface): Promise<TranscriptionOptions> {
  ...
  const audioPath = await question(`❓ Path to audio file (default: ${DEFAULT_AUDIO_PATH}): `) || DEFAULT_AUDIO_PATH;

  // Ask for speech start time - this helps avoid false triggers from background music/noise
  const speechStartStr = await question(`❓ At what second does the actual speech begin? (default: ${SPEECH_START_SECONDS_DEFAULT}): `);
  const speechStartsAtSecond = speechStartStr ? parseFloat(speechStartStr) : SPEECH_START_SECONDS_DEFAULT;
...
async function transcribeAudio(options: TranscriptionOptions) {
  ...
  const adjustedCaptions = captions.map(caption => ({
    ...
    timestampMs: caption.timestampMs ? caption.timestampMs + offsetInMillis : null,
  }));
  ...
  console.info("Transcription complete. Check the captions.json file for the results.");
