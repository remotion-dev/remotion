...
export default function Display({
	...
	readonly result: TranscriptionJson | null;
	...
	useEffect(() => {
		...
		audioRef.current.ontimeupdate = () => {
			...
			if (result && result.transcription) {
				timedTranscriptionIndex = result.transcription.findIndex(
					(v: TranscriptionItemWithTimestamp) =>
						v.offsets.from <= activeTime && v.offsets.to >= activeTime,
				);
			}
			...
			<p>
				<span style={{opacity: 1, fontWeight: 900}}>
					{result.transcription
						.slice(0, transcriptionTextIndex + 1)
						.map(({text}) => text)}
				</span>
				<span style={{opacity: 0.5, fontWeight: 900}}>
					{result.transcription
						.slice(transcriptionTextIndex + 1)
						.map(({text}) => text)
						.join(' ')}
				</span>
			</p>
